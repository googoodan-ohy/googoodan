...
const storyLengthUnit=(!locale.prefix||locale.prefix==='us')?'feet':'metres';
...
const baseRows=rows;rows=function(p,seed,count){const generator=units.find(u=>u.id===(p.generatorUnit||p.unit)),original=baseRows({...p,unit:generator.id},seed,count);return original.map(q=>{
 ...
 if(!p.story)return q;
 if(q.op){const a=q.a,b=q.b,answer=String(q.answer);if(generator.grade<=3){const prompt=q.op==='+'?'You have '+a+' counters and get '+b+' more. How many counters do you have now?':q.op==='−'?'You have '+a+' counters and give away '+b+'. How many counters remain?':q.op==='×'?'There are '+a+' boxes with '+b+' counters in each. How many counters are there altogether?':'Share '+a+' counters equally among '+b+' children. How many counters does each child get?';return {prompt,answer,work:a+' '+q.op+' '+b+' = '+answer};}const prompt=q.op==='+'?'A warehouse has '+a+' '+storyLengthUnit+' of cable. It receives another '+b+' '+storyLengthUnit+'. How many '+storyLengthUnit+' of cable does it have now?':q.op==='−'?'A warehouse has '+a+' '+storyLengthUnit+' of cable. '+b+' '+storyLengthUnit+' are shipped to a customer. How many '+storyLengthUnit+' remain?':q.op==='×'?'A rectangular field is '+a+' '+storyLengthUnit+' long and '+b+' '+storyLengthUnit+' wide. What is its area in square '+storyLengthUnit+'?':answer.includes(' R ')?'Pack '+a+' counters into groups of '+b+'. How many full groups and how many counters remain?':'A warehouse has '+a+' '+storyLengthUnit+' of cable. Each piece is '+b+' '+storyLengthUnit+' long. How many pieces can be made?';return {prompt,answer,work:String(a)+' '+q.op+' '+b+' = '+answer};}
