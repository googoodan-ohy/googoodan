...
types.push({id:'measurement',title:'Measurement Conversion Worksheets',family:'Measurement',group:'Measurement',example:'2 + 3',instruction:'Convert US customary units. Show your work.',customWorksheet:true});
const beforeMeasurement=root.Worksheets.generate;
root.Worksheets.generate=function(id,seed,n=6){if(id!=='measurement')return beforeMeasurement(id,seed,n);let state=seed>>>0;const rand=m=>{state=(Math.imul(state,1664525)+1013904223)>>>0;return Math.floor(state/4294967296*m)},pairs=[['inches','feet',12],['feet','yards',3],['inches','yards',36],['ounces','pounds',16],['cups','pints',2],['pints','quarts',2],['quarts','gallons',4],['cups','gallons',16]];return Array.from({length:n},(_,i)=>{const options=i%3===0?[0,1,2]:i%3===1?[3]:[4,5,6,7],pair=pairs[options[rand(options.length)]],reverse=rand(2),quantity=2+rand(19),from=pair[reverse?1:0],to=pair[reverse?0:1],value=reverse?quantity:quantity*pair[2],answer=reverse?quantity*pair[2]:quantity;return {prompt:'Convert '+value.toLocaleString('en-US')+' '+from+' to '+to+'.',answer:answer.toLocaleString('en-US')+' '+to,reason:'1 '+pair[1]+' = '+pair[2]+' '+pair[0]+'.',a:value,b:pair[2],reverse,op:'custom'};});};
