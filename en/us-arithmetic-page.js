...
menu=function(){groups.innerHTML='<div class="us-grades" role="group" aria-label="Choose a grade">'+['K',1,2,3,4,5,6].map(g=>'<button type="button" data-us-grade="'+g+'" aria-pressed="'+(browseUnit.grade===(g==='K'?0:g))+'">'+(g==='K'?'K':'Grade '+g)+'</button>').join('')+'</div><label for="drill-unit">Choose a topic</label><select id="drill-unit">'+DC.units.filter(u=>u.grade===browseUnit.grade).map(u=>'<option value="'+u.id+'" '+(u.id===browseUnit.id?'selected':'')+'>'+u.number+'. '+u.name+'</option>').join('')+'</select><p class="drill-help">Common Core '+browseUnit.standard+'</p>'+groupNames.map(g=>{const list=browseUnit.drills.filter(p=>p.group===g);return list.length?'<details '+(g===selected.group?'open':'')+'><summary>'+groupLabels[g]+' <span class="practice-count">'+list.length+'</span></summary>'+list.map(p=>'<button class="us-profile" type="button" data-drill="'+p.id+'" aria-pressed="'+(p.id===selected.id)+'">'+'<strong class="drill-example">'+esc(sampleText(p))+'</strong><small class="drill-description">'+esc(p.title)+'</small></button>').join('')+'</details>':''}).join('')+'<p class="drill-help">Practice for selected arithmetic standards, not a complete curriculum. Topic order may differ by state or school. <a href="https://www.thecorestandards.org/Math/Content/">Common Core reference</a></p>';
...
rootInit();scalePage();function rootInit(){document.querySelector('aside h1').textContent='US curriculum arithmetic';document.querySelector('aside>p').textContent='Choose a grade, topic, and practice format.';setProfile(selected.id);}
