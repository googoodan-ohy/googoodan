...
grid.innerHTML=rows.map((q,i)=>qhtml(q,i,answers)).join('');document.querySelector('#mode').textContent=answers?'Answer key':'Worksheet';document.querySelector('#questions').setAttribute('aria-pressed',!answers);document.querySelector('#answers').setAttribute('aria-pressed',answers);document.querySelector('#set').textContent=selectedUnit.id+' · '+count+' problems · '+seed;document.querySelector('#status').textContent=selected.title+' · '+count+' problems';if(document.body.dataset.regionalTitle){document.title=document.body.dataset.regionalTitle;document.querySelector('link[rel=canonical]').href=document.body.dataset.regionalCanonical;}else document.title=selectedUnit.name+' | googoodan.';scalePage();history.replaceState(null,'',gdRegionURL('?unit='+selectedUnit.id+'&drill='+selected.id+'&set='+seed));};
