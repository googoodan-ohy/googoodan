...
menu=function(){groups.innerHTML='<div class="us-grades" role="group" aria-label="Choose a grade">'+(locale.grades||['K',1,2,3,4,5,6]).map(g=>'<button type="button" data-us-grade="'+g+'" aria-pressed="'+(browseUnit.grade===(g==='K'?0:g))+'">'+(locale.gradeLabels?.[g]||(g==='K'?'K':(locale.yearLabel||'Grade')+' '+g))+'</button>').join('')+'</div><label for="drill-unit">Choose a topic</label><select id="drill-unit">'+DC.units.filter(u=>u.grade===browseUnit.grade).map(u=>'<option value="'+u.id+'" '+(u.id===browseUnit.id?'selected':'')+'>'+u.number+'. '+u.name+'</option>').join('')+'</select><p class="drill-help">'+esc((browseUnit.grade===0&&locale.prefix==='eng'?'Early years':locale.referenceLabel)||'Common Core')+' '+browseUnit.standard+'</p>'+groupNames.map(g=>{const list=browseUnit.drills.filter(p=>p.group===g);return list.length?'<details '+(g===selected.group?'open':'')+'><summary>'+groupLabels[g]+' <span class="practice-count">'+list.length+'</span></summary>'+list.map(p=>'<button class="us-profile" type="button" data-drill="'+p.id+'" aria-pressed="'+(p.id===selected.id)+'">'+'<strong class="drill-example">'+esc(sampleText(p))+'</strong><small class="drill-description">'+esc(p.title)+'</small></button>').join('')+'</details>':''}).join('')+'<p class="drill-help">Practice for selected arithmetic objectives. This is not a complete curriculum. <a href="'+esc((browseUnit.grade===0&&locale.earlyYearsSource)||locale.source||'https://www.thecorestandards.org/Math/Content/')+'">Official curriculum reference</a></p>';
 groups.querySelectorAll('[data-us-grade]').forEach(b=>b.onclick=()=>{browseUnit=DC.units.find(u=>u.grade===(b.dataset.usGrade==='K'?0:Number(b.dataset.usGrade)));menu();});groups.querySelector('#drill-unit').onchange=e=>{browseUnit=DC.units.find(u=>u.id===e.target.value);menu();};groups.querySelectorAll('[data-drill]').forEach(b=>b.onclick=()=>navigateProfile(b.dataset.drill));};
function sampleText(p){const q=USArithmetic.rows(p,713,1)[0];if(q.prompt)return q.prompt;if(p.mode==='blank')return [q.mask===0?'□':q.a,q.op,q.mask===1?'□':q.b,'=',q.mask===2?'□':q.answer].join(' ');if(p.mode==='digit')return 'Find the missing digit or digits';return q.a+' '+q.op+' '+q.b+' = '+String(q.answer).replace(' R ',' remainder ');}
...
if(q.prompt||selected.mode==='skill'||selected.mode==='story'){let art='';if(q.groups){const[a,b]=q.groups;art='<div class="dot-groups">'+Array.from({length:b},()=>'<span>'+Array(a).fill('●').join(' ')+'</span>').join('')+'</div>';}html='<div class="drill-prompt">'+fractionText(q.prompt)+'</div>'+art+'<div class="drill-response">'+hide(fractionText(q.answer),show)+'</div>'+hide('<small>'+fractionText(q.work||'')+'</small>',show);}
...
else if(selected.mode==='digit'){const digit=(v,which)=>String(v).split('').map((c,j)=>q.digits.which===which&&q.digits.positions.includes(j)?'<span class="digit-box">'+hide(c,show)+'</span>':'<span class="digit-cell">'+c+'</span>').join('');if(q.op==='÷'){html='<div class="digit-division"><div class="digit-quotient">'+esc(String(q.answer).replace(' R ',' remainder '))+'</div><div>'+digit(q.b,1)+' <span class="dividend-digits">'+digit(q.a,0)+'</span></div></div>';}else html='<div class="digit-vertical"><div>'+digit(q.a,0)+'</div><div class="digit-rule">'+q.op+' '+digit(q.b,1)+'</div><div>'+digit(q.answer,2)+'</div></div>';}
...
if(selected.mode==='story'){cols=2;count=['add-10','add-20'].includes(selectedUnit.id)?12:16;}
...
else if(['add-10','add-20'].includes(selectedUnit.id)){cols=3;count=24;}
return {cols,count};}
render=function(){const theme=KoCatalog.themes[Math.max(0,selectedUnit.grade-1)*2],paper=document.querySelector('.paper');current=types.find(t=>t.id===selected.id);paper.classList.add('drill-sheet');paper.classList.toggle('written-sheet',selected.layout==='vertical');paper.dataset.family=current.family;paper.style.setProperty('--tone',theme[2]);paper.style.setProperty('--wash',theme[3]);
const remedial=selected.group==='기초 보충';document.querySelector('#sheet-title').textContent=(remedial?'[Review] ':'')+selected.title;document.querySelector('.adventure-label').textContent=selectedUnit.gradeLabel+' · '+selectedUnit.standard;document.querySelector('#instructions').textContent=remedial?'Review of prerequisite skills':selected.mode==='story'?'Read each situation and choose the operation.':selected.mode==='digit'||selected.mode==='blank'?'Write the missing number in each box.':'Calculate each answer.';
...
grid.innerHTML=rows.map((q,i)=>qhtml(q,i,answers)).join('');window.GDRegionalEnglish?.apply(document.querySelector('.paper'));document.querySelector('#mode').textContent=answers?'Answer key':'Worksheet';document.querySelector('#questions').setAttribute('aria-pressed',!answers);document.querySelector('#answers').setAttribute('aria-pressed',answers);document.querySelector('#set').textContent=selectedUnit.id+' · '+count+' problems · '+seed;document.querySelector('#status').textContent=selected.title+' · '+count+' problems';if(document.body.dataset.regionalTitle){document.title=document.body.dataset.regionalTitle;document.querySelector('link[rel=canonical]').href=document.body.dataset.regionalCanonical;}else document.title=selectedUnit.name+' | googoodan.';scalePage();history.replaceState(null,'',gdRegionURL('?unit='+selectedUnit.id+'&drill='+selected.id+'&set='+seed));};
