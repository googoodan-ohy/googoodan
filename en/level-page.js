...
let selectedUnit=DC.units.find(u=>u.id===params.get('unit'))||DC.units[0];let selected=selectedUnit.drills.find(p=>p.id===params.get('drill'))||selectedUnit.drills[0];
seed=Number(params.get('set'))||seed;
...
function nav(){return '<nav class="edition-tabs"><a href="/en/">All arithmetic</a><a href="levels.html" aria-current="page">Practice by level</a><a href="grades.html">US curriculum</a></nav>';}
function setProfile(id){selected=DC.profiles.get(id);current=types.find(t=>t.id===id);menu();render();}
menu=function(){groups.innerHTML=nav()+'<label for="drill-unit">Choose a skill level</label><select id="drill-unit">'+DC.units.map(u=>'<option value="'+u.id+'" '+(u.id===selectedUnit.id?'selected':'')+'>'+u.number+'. '+u.name+'</option>').join('')+'</select><p class="drill-help">'+selectedUnit.drills.length+' practice options · Skill levels are not school grades.</p>'+groupNames.map(g=>'<details '+(g===selected.group?'open':'')+'><summary>'+groupLabels[g]+' <small>'+selectedUnit.drills.filter(p=>p.group===g).length+'</small></summary>'+selectedUnit.drills.filter(p=>p.group===g).map(p=>'<button class="drill-choice" data-drill="'+p.id+'" aria-pressed="'+(p.id===selected.id)+'"><strong>'+esc(p.title)+'</strong><small>'+esc(sampleText(p))+'</small></button>').join('')+'</details>').join('');groups.querySelector('#drill-unit').onchange=e=>{selectedUnit=DC.units.find(u=>u.id===e.target.value);setProfile(selectedUnit.drills[0].id)};groups.querySelectorAll('[data-drill]').forEach(b=>b.onclick=()=>setProfile(b.dataset.drill));};
function sampleText(p){const q=DrillEngine.rows(p,713,1)[0];if(q.prompt)return q.prompt;if(p.mode==='blank')return [q.mask===0?'□':q.a,q.op,q.mask===1?'□':q.b,'=',q.mask===2?'□':q.answer].join(' ');if(p.mode==='digit')return 'Find the missing digit or digits';return q.a+' '+q.op+' '+q.b+' = '+String(q.answer).replace(' R ',' remainder ');}
...
const remedial=selected.group==='기초 보충';document.querySelector('#sheet-title').textContent=(remedial?'[Review] ':'')+selected.title;document.querySelector('.adventure-label').textContent='Level '+selectedUnit.number+' · '+selectedUnit.name;document.querySelector('#instructions').textContent=remedial?'Review of prerequisite skills':selected.mode==='story'?'Read each situation and choose the operation.':selected.mode==='digit'||selected.mode==='blank'?'Write the missing number in each box.':'Calculate each answer.';
...
rootInit();scalePage();function rootInit(){document.querySelector('aside h1').textContent='Practice by level';document.querySelector('aside>p').textContent='Build fluency, review skills, and think it through.';setProfile(selected.id);}
