(()=>{
 const $=id=>document.getElementById(id),day=()=>new Date(Date.now()+32400000).toISOString().slice(0,10);const sites=["ko","en","ja","fr","de","es","it"],names=["한국어","영어","일본어","프랑스어","독일어","스페인어","이탈리아어"];let rows=[],shown=[],loaded=false;
 ...
 function render(){
  ...
  for(let t=Date.parse(start);t<=Date.parse(end);t+=86400000){const date=new Date(t).toISOString().slice(0,10),key=monthly?date.slice(0,7):date;if(!map.has(key))map.set(key,{key,...Object.fromEntries(sites.map(s=>[s,0]))});}
  for(const r of rows){const key=monthly?r.day.slice(0,7):r.day;if(map.has(key)&&sites.includes(r.site))map.get(key)[r.site]+=r.loads;}
  shown=[...map.values()].map(r=>({...r,total:site==='all'?sites.reduce((n,s)=>n+r[s],0):r[site]}));
  ...
  for(const r of shown){const tr=document.createElement('tr');for(const value of [r.key,...sites.map(s=>site==='all'||site===s?r[s]:'—'),r.total]){const td=document.createElement('td');td.textContent=typeof value==='number'?value.toLocaleString():value;tr.append(td);}$('statsRows').append(tr);const bar=document.createElement('div');bar.style.height=Math.max(1,r.total/max*100)+'%';bar.title=r.key+': '+r.total+'회';$('statsChart').append(bar);}
 }
