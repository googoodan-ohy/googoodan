...
menu=function(){groups.innerHTML=nav()+'<div class="drill-grades">'+[1,2,3,4,5,6].map(g=>'<button data-grade="'+g+'" aria-pressed="'+(g===browseUnit.grade)+'">'+g+'학년</button>').join('')+'</div><div class="drill-terms">'+[1,2].map(t=>'<button data-term="'+t+'" aria-pressed="'+(t===browseUnit.semester)+'">'+t+'학기</button>').join('')+'</div><label for="drill-unit">단원</label><select id="drill-unit">'+DC.units.filter(u=>u.grade===browseUnit.grade&&u.semester===browseUnit.semester).map(u=>'<option value="'+u.id+'" '+(u.id===browseUnit.id?'selected':'')+'>'+u.number+'. '+u.name+'</option>').join('')+'</select><p class="drill-help">'+browseUnit.drills.length+'종 · 현재 단원에서 필요한 연습을 골라보세요.</p>'+groupNames.map(g=>'<details '+(g===selected.group?'open':'')+'><summary>'+g+' <small>'+browseUnit.drills.filter(p=>p.group===g).length+'</small></summary>'+browseUnit.drills.filter(p=>p.group===g).map(p=>'<button class="drill-choice" data-drill="'+p.id+'" aria-pressed="'+(p.id===selected.id)+'"><strong class="drill-example">'+esc(sampleText(p))+'</strong><small class="drill-description">'+esc(p.title)+'</small></button>').join('')+'</details>').join('');
...
const remedial=selected.group==='기초 보충';document.querySelector('#sheet-title').textContent=(remedial?'[기초 보충] ':'')+selected.title;document.querySelector('.adventure-label').textContent=selectedUnit.grade+'학년 '+selectedUnit.semester+'학기 · '+selectedUnit.number+'. '+selectedUnit.name;document.querySelector('#instructions').textContent=remedial?'현재 단원을 위한 이전 학습 복습':selected.mode==='story'?'상황을 읽고 필요한 계산을 스스로 골라 풀어 보세요.':selected.mode==='digit'||selected.mode==='blank'?'□에 알맞은 수를 쓰세요.':'차근차근 계산하세요.';
