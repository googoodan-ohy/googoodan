...
function sheetHTML(isAnswer){
 ...
 return '<article class="sheet '+(compact?'compact ':'')+(isAnswer?'answer-key':'')+'" data-sections="'+sections.length+'" data-grade="'+grade+'" style="--accent:'+t[2]+';--light:'+t[3]+'"><div class="sheet-top"><span class="worksheet-brand"><small>Des exercices gratuits pour progresser ! · </small><b>googoodan</b> <small>· googoodan.com</small></span><span>'+(['CP','CE1','CE2','CM1','CM2','6e'][grade-1]+' · ')+(isAnswer?'Corrigé et explications':'Ma fiche de maths')+'</span></div><div class="hero"><div><small>'+E(t[0])+'</small><h2>'+unit.number+'. '+E(unit.name)+'</h2><small>'+title()+' · '+E(t[1])+'</small></div>'+mascot(t[4])+'</div><div class="fields"><span>Nom : __________________</span><span>Date : ______________</span></div><div class="activities">'+sections.map((s,i)=>'<section class="activity"><h3><span>'+(i+1)+'</span>'+E(s.title)+'</h3><div class="question-grid">'+s.questions.slice(0,counts[i]).map(q=>qHTML(q,++n)).join('')+'</div></section>').join('')+'</div><footer class="sheet-footer"><b>Des exercices gratuits pour progresser ! · googoodan · googoodan.com</b><span>'+unit.id+' / '+(profile+1)+' · '+seed+'</span></footer><div class="art-credit">Images : Twemoji © Twitter et contributeurs · CC BY 4.0 · googoodan.com/ko/art-gallery.html</div></article>';
}
function fit(){
 ...
 holder.remove();
 if(!best)throw Error('La fiche dépasse une page : '+unit.id+' / '+profile);
...
function draw(){
 ...
 $('#status').textContent=counts.reduce((a,b)=>a+b,0)+' exercices · '+sections.length+' activités · une fiche et un corrigé';
}
function render(){
...
function goProfile(p){if(browseUnit.id===unit.id&&p===profile)return;location.assign('/fr/print/unit-'+browseUnit.id+'-'+p+'.html');}
...
function menus(){
 $('#grades').innerHTML=[...new Set(C.units.map(u=>u.grade))].map(g=>'<button data-g="'+g+'" aria-pressed="'+(g===browseGrade)+'">'+(['','CP','CE1','CE2','CM1','CM2'][g])+'</button>').join('');
