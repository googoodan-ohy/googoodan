...
menu=function(){groups.innerHTML=nav()+'<div class="drill-grades">'+[...new Set(DC.units.map(u=>u.grade))].map(g=>'<button data-grade="'+g+'" aria-pressed="'+(g===browseUnit.grade)+'">'+(DC.units.find(u=>u.grade===g).gradeLabel||g)+'</button>').join('')+'</div><div class="drill-terms">'+[].map(t=>'<button data-term="'+t+'" aria-pressed="'+(t===browseUnit.semester)+'">'+t+' période</button>').join('')+'</div><label for="drill-unit">Chapitre</label><select id="drill-unit">'+DC.units.filter(u=>u.grade===browseUnit.grade&&u.semester===browseUnit.semester).map(u=>'<option value="'+u.id+'" '+(u.id===browseUnit.id?'selected':'')+'>'+u.number+'. '+u.name+'</option>').join('')+'</select><p class="drill-help">'+browseUnit.drills.length+'types · Choisissez votre exercice.</p>'+groupNames.map(g=>'<details '+(g===selected.group?'open':'')+'><summary>'+g+' <small>'+browseUnit.drills.filter(p=>p.group===g).length+'</small></summary>'+browseUnit.drills.filter(p=>p.group===g).map(p=>'<button class="drill-choice" data-drill="'+p.id+'" aria-pressed="'+(p.id===selected.id)+'"><strong class="drill-example">'+esc(sampleText(p).replace(/(\d)\.(\d)/g,'$1,$2'))+'</strong><small class="drill-description">'+esc(p.title)+'</small></button>').join('')+'</details>').join('');
groups.querySelectorAll('[data-grade]').forEach(b=>b.onclick=()=>{browseUnit=DC.units.find(u=>u.grade===+b.dataset.grade&&u.semester===browseUnit.semester)||DC.units.find(u=>u.grade===+b.dataset.grade);menu()});groups.querySelectorAll('[data-term]').forEach(b=>b.onclick=()=>{browseUnit=DC.units.find(u=>u.grade===browseUnit.grade&&u.semester===+b.dataset.term);menu()});groups.querySelector('#drill-unit').onchange=e=>{browseUnit=DC.units.find(u=>u.id===e.target.value);menu()};groups.querySelectorAll('[data-drill]').forEach(b=>b.onclick=()=>navigateProfile(b.dataset.drill));};
function sampleText(p){const q=DrillEngine.rows(p,713,1)[0];if(q.prompt)return q.prompt;if(p.mode==='blank')return [q.mask===0?'□':q.a,q.op,q.mask===1?'□':q.b,'=',q.mask===2?'□':q.answer].join(' ');if(p.mode==='digit')return 'Complétez les chiffres cachés par □';return q.a+' '+q.op+' '+q.b+' = '+String(q.answer).replace(' R ',' reste ');}
function scalar(x){return mathHTML(x);}
function fractionText(s){return esc(s).replace(/(\d+)[と과] (\d+\/\d+)/g,'($1 + $2)').replace(/(\d+) (\d+\/\d+)/g,'($1 + $2)').replace(/(\d+)\uacfc (\d+\/\d+)/g,(_,w,f)=>scalar(w+' '+f)).replace(/(?<![\d>])(\d+)\/(\d+)(?![\d<])/g,(_,n,d)=>scalar(n+'/'+d));}
...
else if(selected.mode==='digit'){const digit=(v,which)=>String(v).split('').map((c,j)=>q.digits.which===which&&q.digits.positions.includes(j)?'<span class="digit-box">'+hide(c,show)+'</span>':'<span class="digit-cell">'+c+'</span>').join('');if(q.op==='÷'){html='<div class="digit-division"><div class="digit-quotient">'+esc(String(q.answer).replace(' R ',' reste '))+'</div><div>'+digit(q.b,1)+' <span class="dividend-digits">'+digit(q.a,0)+'</span></div></div>';}else html='<div class="digit-vertical"><div>'+digit(q.a,0)+'</div><div class="digit-rule">'+q.op+' '+digit(q.b,1)+'</div><div>'+digit(q.answer,2)+'</div></div>';}
...
html=html.replace(/(<span class="mixed-whole">[^<]+<\/span>)(<span class="fraction"><span>[^<]+<\/span><span>[^<]+<\/span><\/span>)/g,'($1 + $2)');if(current.family==='Decimals'||selected.skill==='decimal-scale')html=html.replace(/(\d)\.(\d)/g,'$1,$2');return '<div class="problem"><span class="number">'+(i+1)+'.</span>'+html+'</div>';}
...
const remedial=selected.group==='Révisions';document.querySelector('#sheet-title').textContent=(remedial?'[Révisions] ':'')+selected.title;document.querySelector('.adventure-label').textContent=(selectedUnit.gradeLabel||selectedUnit.grade)+' · '+selectedUnit.number+'. '+selectedUnit.name;document.querySelector('#instructions').textContent=remedial?'Révisez les bases de ce chapitre':selected.mode==='story'?'Lisez le problème et choisissez le calcul.':selected.mode==='digit'||selected.mode==='blank'?'Écrivez le nombre qui manque dans □.':'Effectuez les calculs.';
...
if(count<plan.cols)throw Error('Espace insuffisant sur la fiche: '+selected.id);
grid.innerHTML=rows.map((q,i)=>qhtml(q,i,answers)).join('');document.querySelector('#mode').textContent=answers?'Corrigé':'Fiche';document.querySelector('#questions').setAttribute('aria-pressed',!answers);document.querySelector('#answers').setAttribute('aria-pressed',answers);document.querySelector('#set').textContent=selectedUnit.id+' · '+count+' exercices · '+seed;document.querySelector('#status').textContent=selected.title+' · '+count+' exercices';scalePage();history.replaceState(null,'',window.WORKSHEET_ENTRY?location.pathname:('?unit='+selectedUnit.id+'&drill='+selected.id+'&set='+seed));};
...
rootInit();scalePage();function rootInit(){document.querySelector('aside>p').textContent='Choisissez un calcul ou une révision';setProfile(selected.id);}
