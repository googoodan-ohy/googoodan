...
function navigateProfile(id){if(browseUnit.id===selectedUnit.id&&id===selected.id)return;location.href='/ja/print/drill-'+encodeURIComponent(id)+'.html';}
const groupNames=['基本計算','基礎の復習','穴埋め','文章題'];
function nav(){return '<nav class="edition-tabs"><a href="/ja/"><span class="arithmetic-menu-icons" aria-hidden="true"><span>＋</span><span>−</span><span>×</span><span>÷</span></span><span>四則計算</span></a><a href="/ja/drills.html" aria-current="page">単元別計算</a><a href="/ja/units.html">単元別問題</a></nav>';}
function setProfile(id){selected=DC.profiles.get(id);current=types.find(t=>t.id===id);menu();render();}
menu=function(){groups.innerHTML=nav()+'<div class="drill-grades">'+[1,2,3,4,5,6].map(g=>'<button data-grade="'+g+'" aria-pressed="'+(g===browseUnit.grade)+'">'+g+'年生</button>').join('')+'</div><div class="drill-terms">'+[].map(t=>'<button data-term="'+t+'" aria-pressed="'+(t===browseUnit.semester)+'">'+t+'期</button>').join('')+'</div><label for="drill-unit">単元</label><select id="drill-unit">'+DC.units.filter(u=>u.grade===browseUnit.grade&&u.semester===browseUnit.semester).map(u=>'<option value="'+u.id+'" '+(u.id===browseUnit.id?'selected':'')+'>'+u.number+'. '+u.name+'</option>').join('')+'</select><p class="drill-help">'+browseUnit.drills.length+'種類 · 練習したい内容を選びましょう.</p>'+groupNames.map(g=>'<details '+(g===selected.group?'open':'')+'><summary>'+g+' <small>'+browseUnit.drills.filter(p=>p.group===g).length+'</small></summary>'+browseUnit.drills.filter(p=>p.group===g).map(p=>'<button class="drill-choice" data-drill="'+p.id+'" aria-pressed="'+(p.id===selected.id)+'"><strong class="drill-example">'+esc(sampleText(p))+'</strong><small class="drill-description">'+esc(p.title)+'</small></button>').join('')+'</details>').join('');
...
function sampleText(p){const q=DrillEngine.rows(p,713,1)[0];if(q.prompt)return q.prompt;if(p.mode==='blank')return [q.mask===0?'□':q.a,q.op,q.mask===1?'□':q.b,'=',q.mask===2?'□':q.answer].join(' ');if(p.mode==='digit')return '一つか二つの数字を □で隠します';return q.a+' '+q.op+' '+q.b+' = '+String(q.answer).replace(' R ',' あまり ');}
...
else if(selected.mode==='digit'){const digit=(v,which)=>String(v).split('').map((c,j)=>q.digits.which===which&&q.digits.positions.includes(j)?'<span class="digit-box">'+hide(c,show)+'</span>':'<span class="digit-cell">'+c+'</span>').join('');if(q.op==='÷'){html='<div class="digit-division"><div class="digit-quotient">'+esc(String(q.answer).replace(' R ',' あまり '))+'</div><div>'+digit(q.b,1)+' <span class="dividend-digits">'+digit(q.a,0)+'</span></div></div>';}else html='<div class="digit-vertical"><div>'+digit(q.a,0)+'</div><div class="digit-rule">'+q.op+' '+digit(q.b,1)+'</div><div>'+digit(q.answer,2)+'</div></div>';}
...
const remedial=selected.group==='基礎の復習';document.querySelector('#sheet-title').textContent=(remedial?'[基礎の復習] ':'')+selected.title;document.querySelector('.adventure-label').textContent=selectedUnit.grade+'年生 · '+selectedUnit.number+'. '+selectedUnit.name;document.querySelector('#instructions').textContent=remedial?'この単元のための復習':selected.mode==='story'?'文章を読んで必要な計算を選びましょう.':selected.mode==='digit'||selected.mode==='blank'?'□に当てはまる数を書きましょう.':'順に計算しましょう.';
