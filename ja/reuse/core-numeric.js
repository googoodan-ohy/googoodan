...
if(['add','sub','unknown','three'].includes(kind)){
 ...
 if(kind==='reduce'){a=between(2,5);finish('기약分수로 약分하세요.','<div class="kequation">'+F(n*a,d*a)+'</div>',frac(n,d),'分자와 分모를 最大公約数 '+gcd(n*a,d*a)+'로 나눕니다.',{wrong:(n*a)+'/'+(d*a+1)})}
 ...
 if(kind==='ratio')finish('빨간 구슬 '+a+'개の파란 구슬 '+b+'개に대한 비를 書きましょう.','',a+' : '+b,'비교하は 양を앞에, 기준량を뒤に씁니다.',{wrong:b+' : '+(a+1)});
 ...
 else if(kind==='ratio-compare'){const x=a/b,y=c/b;finish('비율が더 큰 것を選びましょう.','<div class="kequation">가: '+a+' : '+b+'　나: '+c+' : '+b+'</div>',x>y?'가':x<y?'나':'같다','기준량が같으므로 비교하は 양を비교します。',{choices:['가','나','같다'],wrong:x>y?'나':'가'})}
 else if(kind==='proportion')finish('비례식の□를 求めましょう.','<div class="kequation">'+a+' : '+b+' = □ : '+b*c+'</div>',a*c,'두 수に같은 수 '+c+'를 곱します。');
 else if(kind==='share-ratio'){const total=(a+b)*c;finish(total+'개를 '+a+' : '+b+'로 나눌 때 앞쪽 몫은何개ですか。','',a*c,total+' × '+a+'/('+a+' + '+b+') = '+a*c)}
 else if(kind==='ratio-simplify'){const g=gcd(a,b);finish(a*c+' : '+b*c+'를 가장 간단한 자연수の비로 表しましょう.','',a/g+' : '+b/g,'두 수를 最大公約数 '+g*c+'로 나눕니다.',{wrong:a/g+' : '+(b/g+1)})}
 else if(kind==='chance'){const ans=pick(['불가능하다','확실하다','반반이다']),stem=ans==='불가능하다'?'빨간 공만 있は 상자の파란 공を뽑を가능성은 어떤가요?':ans==='확실하다'?'파란 공만 있は 상자の파란 공を뽑を가능성은 어떤가요?':'빨간 공 1개와 파란 공 1개 중 보지 않고 하나를 뽑を때 빨간 공일 가능성은 어떤가요?';finish(stem,'',ans,'상자 안の공 종류와 개수를 비교します。',{choices:['불가능하다','확실하다','반반이다'],wrong:ans==='확실하다'?'불가능하다':'확실하다'})}
 else {a=between(5,15);b=between(1,4);const vals=[a-b,a,a+b];if(kind==='average')finish('세 수の평균を求めましょう.','<div class="kequation">'+vals.join(', ')+'</div>',a,'합 '+a*3+'を자료 수 3으로 나눕니다.');else finish('평균が'+a+'일 때 □를 求めましょう.','<div class="kequation">'+(a-b)+', □, '+(a+b)+'</div>',a,'합 '+a*3+'の알려진 두 수를 뺍니다.')}
}
 ...
 if(kind==='graph-choice')finish('時間に따른 기온の변화를 나타내기に알맞은 그래프は何ですか。','','꺾은선그래프','時間に따라 변하は 모습を선으로 연결します。',{choices:['꺾은선그래프','원그래프','띠그래프'],wrong:'원그래프'});
 else if(mode<2)finish((param==='line'?(idx+1)+'일の기온은何℃':labels[idx]+(param==='pie'||param==='strip'?'の비율은何%':'를 선택한 사람은何명'))+'ですか。',art,vals[idx],'눈금이나 범례の해당 항목の값を읽습니다.');
 else if(mode===2)finish(param==='line'?'첫째 날と둘째 날の기온 합を求めましょう.':'가와 나の값を더하세요.',art,vals[0]+vals[1],vals[0]+' + '+vals[1]+' = '+(vals[0]+vals[1]));
 else if(mode===3)finish('가장 큰 값と가장 작은 값の차이를 求めましょう.',art,Math.max(...vals)-Math.min(...vals),'최댓값の최솟값を뺍니다.');
 else {finish('그래프の자료를 표로 정리하세요.',art,vals.map((v,i)=>(param==='line'?(i+1)+'일':labels[i])+': '+v).join(', '),'항목별 값を표に옮깁니다.',{wrong:'모두 0'});task='<table class="ktable"><tr>'+labels.map(x=>'<th>'+x+'</th>').join('')+'</tr><tr><td>　</td><td>　</td><td>　</td></tr></table>';open=true}
}
 ...
 if(kind==='angle-right'){const yes=r(2)===0;finish('직각인 그림ですか。',svg('<path d="M65 15V67H165" fill="none" stroke="#547b92" stroke-width="3" transform="'+(yes?'':'translate(25 0) skewX(-25)')+'"/>'),yes?'예':'아니요','종이の반듯한 모서리와 겹쳐 확인します。',{choices:['예','아니요'],wrong:yes?'아니요':'예'})}
 ...
 finish(kind==='angle-type'?'각の종류를 選びましょう.':'각도기の눈금を읽어 각도를 求めましょう.',visual,kind==='angle-type'?(angle<90?'예각':angle===90?'직각':'둔각'):angle,kind==='angle-measure'?'오른쪽 0°부터 시작해 다른 변が가리키は 눈금 '+angle+'°를 읽습니다.':'90°보다 작으면 예각, 같으면 직각, 크고 180°보다 작으면 둔각です。',{choices:kind==='angle-type'?['예각','직각','둔각']:undefined,wrong:kind==='angle-type'?(angle===90?'예각':'직각'):angle+10})
}
 else if(kind==='angle-sum'){a=between(2,8)*10;b=between(1,8)*10;finish('이웃한 두 각 '+a+'°와 '+b+'°를 합치면何도ですか。','',a+b,a+' + '+b+' = '+(a+b)+'°')}
 else if(kind==='triangle-angle'||kind==='quad-angle'){const tri=kind==='triangle-angle',sum=tri?180:360,vals=tri?[between(3,7)*10,between(3,7)*10]:[90,90,between(6,12)*10];finish((tri?'三角形':'四角形')+'の각 중 '+vals.map(v=>v+'°').join(', ')+'를 알고 있습니다. あまり 각은何도ですか。','',sum-vals.reduce((x,y)=>x+y),'각の합 '+sum+'°の알려진 각を뺍니다.')}
 ...
 else if(kind==='quad-type'){const idx=r(4),names=['직四角形','정四角形','마름모','평행사변형'],facts=['네 각が직각이고 가로 6 cm, 세로 3 cm','네 각が직각이고 네 변が모두 4 cm','네 변が같고 한 각が60°','마주 보は 두 쌍の변が평행하고 이웃한 변が4 cm와 6 cm, 한 각が60°'];finish(facts[idx]+'인 四角形の이름은 무엇ですか。','',names[idx],'변の長さ와 각の조건を확인します。',{choices:names,wrong:names[(idx+1)%4]})}
 else if(kind==='parallel')finish('직四角形の마주 보は 두 변の관계를 書きましょう.',rect('6 cm','3 cm'),'평행','아무리 늘여도 만나지 않は 두 직선です。',{choices:['평행','수직'],wrong:'수직'});
 else if(kind==='tile'){a=between(2,5);b=between(2,6);finish('한 칸に조각 하나를 놓습니다.何개가 필요한가요?',grid(a,b),a*b,a+' × '+b+' = '+a*b)}
 else {const n=between(3,8);if(kind==='diagonal')finish('한 꼭짓점の그を수 있は 대각선은何개ですか。',poly(n),n-3,'자기 자신と이웃한 두 꼭짓점を제외します。');else finish('다각형の변은何개ですか。',poly(n),n,'둘레를 따라 변を하나씩 셉니다.')}
}
 ...
 if(kind==='circle-parts')finish('중심の원 위の한 점까지 이은 선分の이름은 무엇ですか。',visual,'반지름','중심の원 위の점까지 이은 선分です。',{choices:['반지름','지름','꼭짓점'],wrong:'지름'});
 else if(kind==='circle-size')finish('원の지름은何cmですか。',visual,a*2,'지름은 반지름の2なしです。');
 else if(kind==='circle-draw')finish('반지름 '+a+' cm인 원を그리려면 컴퍼스 두 발 사이를何cm로 벌려야 하나요?','',a,'컴퍼스 두 발 사이は 반지름と같습니다.');
 else {const result=kind==='circle-circumference'?2*a*3.14:kind==='circle-half'?a*a*3.14/2:a*a*3.14;finish((kind==='circle-circumference'?'원の둘레(cm)':kind==='circle-half'?'が원の절반인 반원の広さ(cm²)':'원の広さ(cm²)')+'를 求めましょう. (원주율: 3.14)',visual,Number(result.toFixed(2)),kind==='circle-circumference'?'지름 × 3.14 = '+a*2+' × 3.14':'반지름 × 반지름 × 3.14'+(kind==='circle-half'?' ÷ 2':''))}
}
 ...
 else if(kind==='symmetry-line')finish('정四角形の대칭축은何개ですか。',poly(4),4,'마주 보は 변の중점を잇は 2개와 대각선 2個です。');
 ...
 if(kind==='perimeter'){result=2*(a+b);stem='직四角形の둘레は何cmですか。';art=rect(a+' cm',b+' cm');why='(가로 + 세로) × 2'}
 ...
 else {const result=kind==='cube-view'?(front?Math.max(hs[0],hs[2])+Math.max(hs[1],hs[3]):Math.max(hs[0],hs[1])+Math.max(hs[2],hs[3])):kind==='cube-missing'?sum-hs[0]:sum;finish(kind==='cube-view'?(front?'앞':'오른쪽 옆')+'の보이は 정四角形은何개ですか。':kind==='cube-missing'?'왼쪽 위 자리の쌓기나무를 모두 빼면何개가 남나요?':'쌓기나무は 모두何개ですか。',visual,result,kind==='cube-view'?'보は 방향の겹친 줄마다 가장 높은 개수をたすと '+result+'個です。':hs.join(' + ')+' = '+sum+(kind==='cube-missing'?', '+sum+' − '+hs[0]+' = '+result:''))}
}
 else if(kind==='cube-net'){const cells=[[1,0],[0,1],[1,1],[2,1],[3,1],[1,2]],idx=r(6);visual=svg(cells.map(([x,y],i)=>'<rect x="'+(30+x*24)+'" y="'+(5+y*24)+'" width="24" height="24" fill="'+(i===idx?'#efca80':'#e7eff5')+'" stroke="#64879a"/>').join(''),170,85);finish('정육면체 전개도の색칠하지 않은 면은何개ですか。',visual,5,'정四角形 6개 중 색칠한 1개를 빼면 5個です。');if(mode===4){prompt='が전개도를 접어 정육면체가 되は 까닭を설명하세요.';answer='6개の정四角形が겹치지 않고 정육면체の6개 면を만듭니다.';reason='직접 오려 접어 확인해도 좋습니다.'}}
 else if(['cuboid','cuboid-edges'].includes(kind)){const attr=kind==='cuboid-edges'?['모서리',12]:kind==='cube-net'?['전개도に필요한 정四角形',6]:pick([['면',6],['꼭짓점',8],['모서리',12]]);finish((kind==='cube-net'?'정육면체':'直方体')+'の'+attr[0]+'은何개ですか。',cubeArt(),attr[1],'면 6개, 꼭짓점 8개, 모서리 12個です。')}
 ...
 else if(['prism','pyramid'].includes(kind)){const n=between(3,6),pyr=kind==='pyramid',attr=pick(['면','꼭짓점','모서리']),result=attr==='면'?(pyr?n+1:n+2):attr==='꼭짓점'?(pyr?n+1:2*n):(pyr?2*n:3*n),name=['','','','삼','사','오','육'][n]+(pyr?'각뿔':'각기둥');finish(name+'の'+attr+'은何개ですか。','',result,pyr?'밑면と꼭대기 꼭짓점を구分して셉니다.':'위아래 밑면と옆면を구分して셉니다.')}
 else {const facts={cylinder:['円柱の밑면은何개ですか。',2,'합동인 원 2個です。'],cone:['원뿔の꼭짓점은何개ですか。',1,'꼭대기 꼭짓점은 1個です。'],sphere:['구の중심の겉면까지の거리は 무엇이라고 하나요?','반지름','반지름은 어느 방향이나 같습니다.'],'cylinder-net':['円柱の옆면を높が방향으로 잘라 펼치면 무슨 모양ですか。','직四角形','평행한 두 밑면 사이를 높が방향으로 자릅니다.']};const [stem,result,why]=facts[kind];finish(stem,'',result,why,{wrong:typeof result==='number'?result+1:'三角形',choices:typeof result==='number'?undefined:[result,'三角形','꼭짓점']})}
}
...
'large-number':'万・億・兆','angle-measure':'각도기 읽기','angle-sum':'각도의 합','triangle-angle':'삼각형의 각','quad-angle':'사각형의 각','div-two':'二けたでわる',move:'밀어서 옮기기',flip:'뒤집기',turn:'돌리기',equal:'等しい式',
