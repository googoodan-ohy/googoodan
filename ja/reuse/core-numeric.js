...
if(['add','sub','unknown','three'].includes(kind)){
 ...
 else {const facts={cylinder:['円柱の밑면はいくつですか。',2,'합동인 원 2個です。'],cone:['원뿔の頂点はいくつですか。',1,'꼭대기 頂点은 1個です。'],sphere:['구の중심の겉면까지の거리は 무엇이라고 하나요?','半径','半径은 어느 방향이나 같습니다.'],'cylinder-net':['円柱の옆면を高さ 방향으로 잘라 펼치면 무슨 모양ですか。','長方形','平行한 두 밑면 사이를 高さ 방향으로 자릅니다.']};const [stem,result,why]=facts[kind];finish(stem,'',result,why,{wrong:typeof result==='number'?result+1:'三角形',choices:typeof result==='number'?undefined:[result,'三角形','頂点']})}
}
