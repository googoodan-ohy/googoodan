...
if(['add','sub','unknown','three'].includes(kind)){
 ...
 else {const n=between(3,8);if(kind==='diagonal')finish('한 頂点の그を수 있は 대각선은何개ですか。',poly(n),n-3,'자기 자신と이웃한 두 頂点を제외します。');else finish('다각형の변은何개ですか。',poly(n),n,'둘레를 따라 변を하나씩 셉니다.')}
}
 ...
 else if(['cuboid','cuboid-edges'].includes(kind)){const attr=kind==='cuboid-edges'?['모서리',12]:kind==='cube-net'?['전개도に필요한 정四角形',6]:pick([['면',6],['頂点',8],['모서리',12]]);finish((kind==='cube-net'?'정육면체':'直方体')+'の'+attr[0]+'은何개ですか。',cubeArt(),attr[1],'면 6개, 頂点 8개, 모서리 12個です。')}
 ...
 else if(['prism','pyramid'].includes(kind)){const n=between(3,6),pyr=kind==='pyramid',attr=pick(['면','頂点','모서리']),result=attr==='면'?(pyr?n+1:n+2):attr==='頂点'?(pyr?n+1:2*n):(pyr?2*n:3*n),name=['','','','삼','사','오','육'][n]+(pyr?'각뿔':'각기둥');finish(name+'の'+attr+'은何개ですか。','',result,pyr?'밑면と꼭대기 頂点を구分して셉니다.':'위아래 밑면と옆면を구分して셉니다.')}
 else {const facts={cylinder:['円柱の밑면은何개ですか。',2,'합동인 원 2個です。'],cone:['원뿔の頂点은何개ですか。',1,'꼭대기 頂点은 1個です。'],sphere:['구の중심の겉면까지の거리は 무엇이라고 하나요?','半径','半径은 어느 방향이나 같습니다.'],'cylinder-net':['円柱の옆면を높が방향으로 잘라 펼치면 무슨 모양ですか。','직四角形','평행한 두 밑면 사이를 높が방향으로 자릅니다.']};const [stem,result,why]=facts[kind];finish(stem,'',result,why,{wrong:typeof result==='number'?result+1:'三角形',choices:typeof result==='number'?undefined:[result,'三角形','頂点']})}
}
