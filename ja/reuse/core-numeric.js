...
function calc(x,y,op,opts={}){
 ...
 finish('計算しましょう。',opts.art||'',value,expr+' = '+value,{wrong:fmt(result+1),story:op==='+'?'구슬 '+fmt(x)+'개に'+fmt(y)+'개를 더 넣었습니다. 모두何개ですか。':op==='−'?'색종が'+fmt(x)+'장 중 '+fmt(y)+'장を썼습니다.何장が남았나요?':op==='×'?'한 묶음に'+fmt(x)+'こずつ'+fmt(y)+'묶음です。 모두何개ですか。':fmt(x)+'개를 '+fmt(y)+'명에게 똑같が나누면 한 명이何개를 받나요?'});
...
if(['add','sub','unknown','three'].includes(kind)){
 ...
 if(kind==='div-remainder'){const rem=between(1,b-1),total=a*b+rem;finish(total+' ÷ '+b+'の商とあまりを求めましょう。','',a+' あまり '+rem,b+' × '+a+' + '+rem+' = '+total,{wrong:(a+1)+' あまり '+rem,story:'공 '+total+'개를 '+b+'こずつ상자に담습니다. 가득 찬 상자は何개이고何개가 남나요?'});check={kind:'remainder',total,divisor:b,quotient:a,remainder:rem}}
 ...
 if(kind==='common-denominator'){const e=d+1,l=d*e/gcd(d,e);finish('가장 작은 공통分모로 통分하세요.','<div class="kequation">'+F(n,d)+' , '+F(1,e)+'</div>',(n*l/d)+'/'+l+', '+(l/e)+'/'+l,'分모의 최소공なし수は '+l+'です。',{wrong:n+'/'+l+', 1/'+l})}
}
 ...
 finish(u.grade===4?'計算しましょう。':'計算して、約分した分数か整数で答えましょう。','<div class="kequation">'+F(n,d)+' '+op+' '+F(m,e)+'</div>',ans,expr+' = '+num+'/'+den+' = '+ans,{wrong:frac(num+1,den),story:op==='+'?'리본 '+n+'/'+d+' m와 '+m+'/'+e+' m를 이으면何mですか。':op==='−'?n+'/'+d+' m 중 '+m+'/'+e+' m를 쓰면何m가 남나요?':op==='×'?n+'/'+d+'의 '+m+'/'+e+'なしは 얼마ですか。':n+'/'+d+' m를 '+m+'/'+e+' m씩 나누면何조각 分량ですか。'});
 ...
 if(kind.startsWith('mixed')){const bracket=kind==='mixed-bracket'||mode===4,value=bracket?(a+b)*c:a+b*c,expr=bracket?'('+a+' + '+b+') × '+c:a+' + '+b+' × '+c;finish('순서に따라 計算しましょう。','<div class="kequation">'+expr+'</div>',value,bracket?'괄호 안を먼저 계산한 뒤 곱합니다.':'곱셈を먼저 계산한 뒤 더합니다.',{story:bracket?'연필 '+a+'자루와 '+b+'자루를 한 상자に담아 '+c+'상자를 준비했습니다. 모두何자루ですか。':'연필 '+a+'자루に'+b+'자루씩 든 묶음 '+c+'개를 더했습니다. 모두何자루ですか。'})}
 else if(kind==='factor'){a=between(4,36);const f=Array.from({length:a},(_,i)=>i+1).filter(i=>a%i===0);finish(a+'의 약수를 모두 書きましょう.','',f.join(', '),a+'を나누어떨어지게 하は 자연수를 찾습니다.',{wrong:f.slice(1).join(', ')})}
 else if(kind==='multiple'){a=between(2,12);finish(a+'의 なし수를 작은 수부터 3개 書きましょう.','',[a,a*2,a*3].join(', '),a+'に1, 2, 3を각각 곱합니다.',{wrong:[a,a+1,a+2].join(', ')})}
 else if(kind==='gcd'||kind==='lcm'){a=between(4,24);b=between(4,24);const ans=kind==='gcd'?gcd(a,b):a*b/gcd(a,b);finish(a+'과 '+b+'의 '+(kind==='gcd'?'최대공약수':'최소공なし수')+'は 무엇ですか。','',ans,kind==='gcd'?'공통인 약수 중 가장 큰 수를 찾습니다.':'공통인 なし수 중 가장 작은 수를 찾습니다.')}
 else if(kind==='range'){a=between(10,30);const labels=['이상','이하','초과','미만'],idx=r(4),vals=[a-1,a,a+1],ok=vals.filter(n=>idx===0?n>=a:idx===1?n<=a:idx===2?n>a:n<a);finish(a+' '+labels[idx]+'인 수를 모두 書きましょう.','<div class="kequation">'+vals.join(', ')+'</div>',ok.join(', '),'이상·이하は 기준を포함하고 초과·미만은 포함하지 않습니다.',{wrong:vals.filter(n=>!ok.includes(n)).join(', ')})}
 ...
 if(kind==='ratio')finish('빨간 구슬 '+a+'개의 파란 구슬 '+b+'개に대한 비를 書きましょう.','',a+' : '+b,'비교하は 양を앞에, 기준량を뒤に씁니다.',{wrong:b+' : '+(a+1)});
 else if(kind==='percent'){const total=pick([20,25,50,100]),part=between(1,total);finish('全部で'+total+'개 중 '+part+'개は何%ですか。','',Number((part/total*100).toFixed(6)),'비교하は 양 ÷ 기준량 × 100です。')}
 else if(kind==='ratio-compare'){const x=a/b,y=c/b;finish('비율が더 큰 것を選びましょう.','<div class="kequation">가: '+a+' : '+b+'　나: '+c+' : '+b+'</div>',x>y?'가':x<y?'나':'같다','기준량が같으므로 비교하は 양を비교합니다.',{choices:['가','나','같다'],wrong:x>y?'나':'가'})}
 else if(kind==='proportion')finish('비례식의 □를 求めましょう.','<div class="kequation">'+a+' : '+b+' = □ : '+b*c+'</div>',a*c,'두 수に같은 수 '+c+'를 곱합니다.');
 else if(kind==='share-ratio'){const total=(a+b)*c;finish(total+'개를 '+a+' : '+b+'로 나눌 때 앞쪽 몫은何개ですか。','',a*c,total+' × '+a+'/('+a+' + '+b+') = '+a*c)}
 ...
 else if(kind==='chance'){const ans=pick(['불가능하다','확실하다','반반이다']),stem=ans==='불가능하다'?'빨간 공만 있は 상자の파란 공を뽑を가능성은 어떤가요?':ans==='확실하다'?'파란 공만 있は 상자の파란 공を뽑を가능성은 어떤가요?':'빨간 공 1개와 파란 공 1개 중 보지 않고 하나를 뽑を때 빨간 공일 가능성은 어떤가요?';finish(stem,'',ans,'상자 안의 공 종류와 개수를 비교합니다.',{choices:['불가능하다','확실하다','반반이다'],wrong:ans==='확실하다'?'불가능하다':'확실하다'})}
 ...
 else if(param==='picture'){const scale=u.grade===2?1:10;if(scale===1)vals.forEach((v,i)=>vals[i]=v/10);art=svg(vals.map((v,i)=>text(15,22+i*25,labels[i])+Array.from({length:v/(u.grade===2?1:10)},(_,j)=>'<circle cx="'+(48+j*25)+'" cy="'+(18+i*25)+'" r="7" fill="#8fc6bc"/>').join('')).join('')+text(120,100,'● 한 개は '+scale+'명'),240,112);}
 ...
 if(kind==='graph-choice')finish('時間に따른 기온의 변화를 나타내기に알맞은 그래프は 무엇ですか。','','꺾은선그래프','時間に따라 변하は 모습を선으로 연결합니다.',{choices:['꺾은선그래프','원그래프','띠그래프'],wrong:'원그래프'});
 else if(mode<2)finish((param==='line'?(idx+1)+'일의 기온은何℃':labels[idx]+(param==='pie'||param==='strip'?'의 비율은何%':'를 선택한 사람은何명'))+'ですか。',art,vals[idx],'눈금이나 범례の해당 항목의 값を읽습니다.');
 ...
 else {finish('그래프의 자료를 표로 정리하세요.',art,vals.map((v,i)=>(param==='line'?(i+1)+'일':labels[i])+': '+v).join(', '),'항목별 값を표に옮깁니다.',{wrong:'모두 0'});task='<table class="ktable"><tr>'+labels.map(x=>'<th>'+x+'</th>').join('')+'</tr><tr><td>　</td><td>　</td><td>　</td></tr></table>';open=true}
}
 ...
 finish(kind==='angle-type'?'각의 종류를 選びましょう.':'각도기의 눈금を읽어 각도를 求めましょう.',visual,kind==='angle-type'?(angle<90?'예각':angle===90?'직각':'둔각'):angle,kind==='angle-measure'?'오른쪽 0°부터 시작해 다른 변が가리키は 눈금 '+angle+'°를 읽습니다.':'90°보다 작으면 예각, 같으면 직각, 크고 180°보다 작으면 둔각です。',{choices:kind==='angle-type'?['예각','직각','둔각']:undefined,wrong:kind==='angle-type'?(angle===90?'예각':'직각'):angle+10})
}
 else if(kind==='angle-sum'){a=between(2,8)*10;b=between(1,8)*10;finish('이웃한 두 각 '+a+'°와 '+b+'°를 합치면何도ですか。','',a+b,a+' + '+b+' = '+(a+b)+'°')}
 else if(kind==='triangle-angle'||kind==='quad-angle'){const tri=kind==='triangle-angle',sum=tri?180:360,vals=tri?[between(3,7)*10,between(3,7)*10]:[90,90,between(6,12)*10];finish((tri?'三角形':'四角形')+'의 각 중 '+vals.map(v=>v+'°').join(', ')+'를 알고 있습니다. あまり 각은何도ですか。','',sum-vals.reduce((x,y)=>x+y),'각의 합 '+sum+'°の알려진 각を뺍니다.')}
 else if(kind==='triangle-side'){a=between(3,9);finish('세 변が모두 '+a+' cm인 三角形의 이름은 무엇ですか。',poly(3),'정三角形','세 변의 長さ가 모두 같습니다.',{choices:['정三角形','직각三角形','둔각三角形'],wrong:'둔각三角形'})}
 else if(kind==='triangle-type'){const idx=r(3),angles=[[60,60,60],[30,60,90],[30,30,120]][idx];finish('세 각が'+angles.join('°, ')+'°인 三角形を각に따라 分류하세요.','',['예각三角形','직각三角形','둔각三角形'][idx],'직각 또は 둔각が있は지 확인합니다.',{choices:['예각三角形','직각三角形','둔각三角形'],wrong:['둔각三角形','예각三角形','직각三角形'][idx]})}
 else if(kind==='quad-type'){const idx=r(4),names=['직四角形','정四角形','마름모','평행사변형'],facts=['네 각が직각이고 가로 6 cm, 세로 3 cm','네 각が직각이고 네 변が모두 4 cm','네 변が같고 한 각が60°','마주 보は 두 쌍의 변が평행하고 이웃한 변が4 cm와 6 cm, 한 각が60°'];finish(facts[idx]+'인 四角形의 이름은 무엇ですか。','',names[idx],'변의 長さ와 각의 조건を확인합니다.',{choices:names,wrong:names[(idx+1)%4]})}
 else if(kind==='parallel')finish('직四角形の마주 보は 두 변의 관계를 書きましょう.',rect('6 cm','3 cm'),'평행','아무리 늘여도 만나지 않は 두 직선です。',{choices:['평행','수직'],wrong:'수직'});
 else if(kind==='tile'){a=between(2,5);b=between(2,6);finish('한 칸に조각 하나를 놓습니다.何개가 필요한가요?',grid(a,b),a*b,a+' × '+b+' = '+a*b)}
 else {const n=between(3,8);if(kind==='diagonal')finish('한 꼭짓점の그を수 있は 대각선은何개ですか。',poly(n),n-3,'자기 자신과 이웃한 두 꼭짓점を제외합니다.');else finish('다각형의 변은何개ですか。',poly(n),n,'둘레를 따라 변を하나씩 셉니다.')}
}
 ...
 if(kind==='circle-parts')finish('중심の원 위의 한 점까지 이은 선分의 이름은 무엇ですか。',visual,'반지름','중심の원 위의 점까지 이은 선分です。',{choices:['반지름','지름','꼭짓점'],wrong:'지름'});
 else if(kind==='circle-size')finish('원의 지름은何cmですか。',visual,a*2,'지름은 반지름의 2なしです。');
 else if(kind==='circle-draw')finish('반지름 '+a+' cm인 원を그리려면 컴퍼스 두 발 사이를何cm로 벌려야 하나요?','',a,'컴퍼스 두 발 사이は 반지름과 같습니다.');
 ...
 else if(kind==='congruent'){a=between(3,9);finish('합동인 두 三角形の한 변が'+a+' cmです。 대응하は 변은何cmですか。',poly(3),a,'대응변의 長さは 같습니다.')}
 else if(kind==='symmetry-line')finish('정四角形의 대칭축은何개ですか。',poly(4),4,'마주 보は 변의 중점を잇は 2개와 대각선 2個です。');
 else finish('점대칭인 도형を대칭의 중심 둘레로何도 돌리면 처음과 겹치나요?','',180,'반 바퀴は 180°です。');
}
 ...
 if(kind==='perimeter'){result=2*(a+b);stem='직四角形의 둘레は何cmですか。';art=rect(a+' cm',b+' cm');why='(가로 + 세로) × 2'}
 else if(kind==='surface'){result=2*(a*b+b*c+a*c);stem='가로 '+a+' cm, 세로 '+b+' cm, 높が'+c+' cm인 直方体의 겉広さは何cm²ですか。';art=cubeArt();why='(가로×세로 + 세로×높が+ 가로×높이) × 2'}
 else if(kind==='volume'){result=a*b*c;stem='가로 '+a+' cm, 세로 '+b+' cm, 높が'+c+' cm인 直方体의 부피は何cm³ですか。';art=cubeArt();why='가로 × 세로 × 높이'}
 else if(kind==='volume-convert'){result=a*1000000;stem=a+' m³は何cm³ですか。';art='';why='1 m³ = 1,000,000 cm³'}
 else {b=2*between(2,5);if(kind==='area-trapezoid')c=a+between(1,5);result=kind==='area-triangle'?a*b/2:kind==='area-trapezoid'?(a+c)*b/2:kind==='area-rhombus'?a*b/2:a*b;stem=kind==='area-triangle'?'밑변 '+a+' cm, 높が'+b+' cm인 三角形':kind==='area-trapezoid'?'윗변 '+a+' cm, 아랫변 '+c+' cm, 높が'+b+' cm인 사다리꼴':kind==='area-rhombus'?'두 대각선が'+a+' cm, '+b+' cm인 마름모':kind==='area-parallelogram'?'밑변 '+a+' cm, 높が'+b+' cm인 평행사변형':'가로 '+a+' cm, 세로 '+b+' cm인 직四角形';stem+='의 広さは何cm²ですか。';art=kind==='area-rectangle'?rect(a+' cm',b+' cm'):svg('<path d="'+(kind==='area-triangle'?'M35 70L95 12L180 70Z':kind==='area-trapezoid'?'M35 70L65 15H140L180 70Z':kind==='area-rhombus'?'M35 43L105 10L180 43L105 77Z':'M35 70L65 15H185L155 70Z')+'" fill="#e6edf8" stroke="#6d86a5" stroke-width="2"/>'+text(110,91,'長さは 문제의 조건を보세요'),220,100);why=kind==='area-triangle'?'밑변 × 높が÷ 2':kind==='area-trapezoid'?'(윗변 + 아랫변) × 높が÷ 2':kind==='area-rhombus'?'대각선 × 다른 대각선 ÷ 2':'밑변(가로) × 높이(세로)'}
 ...
 visual='<div class="height-map">'+hs.map(n=>'<span>'+n+'</span>').join('')+'</div><small class="diagram-note">위の본 모양 · 숫자は 그 자리의 개수 · 아래쪽が앞</small>';
 ...
 else {const result=kind==='cube-view'?(front?Math.max(hs[0],hs[2])+Math.max(hs[1],hs[3]):Math.max(hs[0],hs[1])+Math.max(hs[2],hs[3])):kind==='cube-missing'?sum-hs[0]:sum;finish(kind==='cube-view'?(front?'앞':'오른쪽 옆')+'の보이は 정四角形은何개ですか。':kind==='cube-missing'?'왼쪽 위 자리의 쌓기나무를 모두 빼면何개가 남나요?':'쌓기나무は 모두何개ですか。',visual,result,kind==='cube-view'?'보は 방향の겹친 줄마다 가장 높은 개수をたすと '+result+'個です。':hs.join(' + ')+' = '+sum+(kind==='cube-missing'?', '+sum+' − '+hs[0]+' = '+result:''))}
}
 else if(kind==='cube-net'){const cells=[[1,0],[0,1],[1,1],[2,1],[3,1],[1,2]],idx=r(6);visual=svg(cells.map(([x,y],i)=>'<rect x="'+(30+x*24)+'" y="'+(5+y*24)+'" width="24" height="24" fill="'+(i===idx?'#efca80':'#e7eff5')+'" stroke="#64879a"/>').join(''),170,85);finish('정육면체 전개도の색칠하지 않은 면은何개ですか。',visual,5,'정四角形 6개 중 색칠한 1개를 빼면 5個です。');if(mode===4){prompt='が전개도를 접어 정육면체가 되は 까닭を설명하세요.';answer='6개의 정四角形が겹치지 않고 정육면체의 6개 면を만듭니다.';reason='직접 오려 접어 확인해도 좋습니다.'}}
 else if(['cuboid','cuboid-edges'].includes(kind)){const attr=kind==='cuboid-edges'?['모서리',12]:kind==='cube-net'?['전개도に필요한 정四角形',6]:pick([['면',6],['꼭짓점',8],['모서리',12]]);finish((kind==='cube-net'?'정육면체':'直方体')+'의 '+attr[0]+'은何개ですか。',cubeArt(),attr[1],'면 6개, 꼭짓점 8개, 모서리 12個です。')}
 else if(kind==='prism-net'){visual=svg('<path d="M28 31H124V71H28ZM60 31V71M92 31V71M28 31L44 4L60 31M28 71L44 98L60 71" fill="#dce9f3" stroke="#62879c"/>',155,106);finish('が전개도를 접어 만든 입체形の名前を書きましょう。',visual,'삼각기둥','三角形인 밑면 2개와 직四角形인 옆면 3個です。',{choices:['삼각기둥','삼각뿔','사각기둥'],wrong:'삼각뿔'})}
 else if(['prism','pyramid'].includes(kind)){const n=between(3,6),pyr=kind==='pyramid',attr=pick(['면','꼭짓점','모서리']),result=attr==='면'?(pyr?n+1:n+2):attr==='꼭짓점'?(pyr?n+1:2*n):(pyr?2*n:3*n),name=['','','','삼','사','오','육'][n]+(pyr?'각뿔':'각기둥');finish(name+'의 '+attr+'은何개ですか。','',result,pyr?'밑면과 꼭대기 꼭짓점を구分하여 셉니다.':'위아래 밑면과 옆면を구分하여 셉니다.')}
 else {const facts={cylinder:['円柱의 밑면은何개ですか。',2,'합동인 원 2個です。'],cone:['원뿔의 꼭짓점은何개ですか。',1,'꼭대기 꼭짓점은 1個です。'],sphere:['구의 중심の겉면까지의 거리は 무엇이라고 하나요?','반지름','반지름은 어느 방향이나 같습니다.'],'cylinder-net':['円柱의 옆면を높が방향으로 잘라 펼치면 무슨 모양ですか。','직四角形','평행한 두 밑면 사이를 높が방향으로 자릅니다.']};const [stem,result,why]=facts[kind];finish(stem,'',result,why,{wrong:typeof result==='number'?result+1:'三角形',choices:typeof result==='number'?undefined:[result,'三角形','꼭짓점']})}
}
...
sort:'種類に分ける',table:'表を読む',graph:'그래프 해석',groups:'まとまりとかけ算',mul:'かけ算',array:'並び方とかけ算','unknown-mul':'かけ算の□',clock:'時計を読む',elapsed:'時間を求める',calendar:'週と日',pattern:'繰り返すきまり','array-pattern':'表のきまり',
...
'fraction-div-whole':'分数を整数でわる','whole-div-fraction-result':'商を分数で表す',prism:'각기둥의 구성',pyramid:'각뿔의 구성','prism-net':'전개도로 입체 찾기','decimal-div-whole':'小数を整数でわる','decimal-fraction':'分数と小数',ratio:'비로 나타내기',percent:'백분율 구하기','ratio-compare':'비율 비교','graph-choice':'알맞은 그래프 선택',surface:'직육면체의 겉넓이',volume:'직육면체의 부피','volume-convert':'부피 単位を変える',
