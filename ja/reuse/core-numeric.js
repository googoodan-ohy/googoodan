...
if(['add','sub','unknown','three'].includes(kind)){
 ...
 else if(kind==='cube-net'){const cells=[[1,0],[0,1],[1,1],[2,1],[3,1],[1,2]],idx=r(6);visual=svg(cells.map(([x,y],i)=>'<rect x="'+(30+x*24)+'" y="'+(5+y*24)+'" width="24" height="24" fill="'+(i===idx?'#efca80':'#e7eff5')+'" stroke="#64879a"/>').join(''),170,85);finish('정육면체 전개도の색칠하지 않은 면은何개ですか。',visual,5,'정四角形 6個のうち색칠한 1個を빼면 5個です。');if(mode===4){prompt='が전개도를 접어 정육면체가 되は 까닭を설人하세요.';answer='6개の정四角形が겹치지 않고 정육면체の6개 면を만듭니다.';reason='직접 오려 접어 확인해도 좋습니다.'}}
