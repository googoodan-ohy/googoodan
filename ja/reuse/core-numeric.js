...
if(['add','sub','unknown','three'].includes(kind)){
 ...
 else if(kind==='symmetry-line')finish('정四角形の대칭축はいくつですか。',poly(4),4,'마주 보は 변の중점を잇は 2개와 대각선 2個です。');
 ...
 else if(['cuboid','cuboid-edges'].includes(kind)){const attr=kind==='cuboid-edges'?['辺',12]:kind==='cube-net'?['전개도に필요한 정四角形',6]:pick([['面',6],['頂点',8],['辺',12]]);finish((kind==='cube-net'?'立方体':'直方体')+'の'+attr[0]+'はいくつですか。',cubeArt(),attr[1],'面は6つ、頂点は8つ、辺は12本です。')}
 ...
 else {const facts={cylinder:['円柱の밑면はいくつですか。',2,'합동인 원 2個です。'],cone:['원뿔の頂点はいくつですか。',1,'꼭대기 頂点은 1個です。'],sphere:['구の중심の겉면까지の거리は 무엇이라고 하나요?','半径','半径은 어느 방향이나 같습니다.'],'cylinder-net':['円柱の옆면を높が방향으로 잘라 펼치면 무슨 모양ですか。','직四角形','평행한 두 밑면 사이를 높が방향으로 자릅니다.']};const [stem,result,why]=facts[kind];finish(stem,'',result,why,{wrong:typeof result==='number'?result+1:'三角形',choices:typeof result==='number'?undefined:[result,'三角形','頂点']})}
}
