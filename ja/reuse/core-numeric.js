...
if(['add','sub','unknown','three'].includes(kind)){
 ...
 if(kind==='three'){c=between(1,Math.max(1,Math.floor(max/3)));a=between(1,c);b=between(1,c);finish('三つの数の和を求めましょう.','<div class="kequation">'+a+' + '+b+' + '+c+'</div>',a+b+c,a+' + '+b+' + '+c+' = '+(a+b+c),{story:'세 바구니에 りんご가 '+a+'개, '+b+'개, '+c+'개 있습니다. 모두 몇 개인가요?'})}
 ...
 else if(kind==='time-seconds'){a=between(1,4);b=between(1,59);finish(a+'分 '+b+'초는 몇 초인가요?','',a*60+b,a+' × 60 + '+b+'です。')}
 else {a=between(1,10);b=r(6)*10;const delta=between(1,6)*10,total=a*60+b+delta;finish(a+'時'+b+'分부터 '+delta+'分 뒤의 시각은 언제인가요?',clock(a,b),Math.floor(total/60)+'時'+total%60+'分','60分은 1時間です。',{wrong:a+'時'+b+'分'})}
}
 ...
 else if(kind==='multiple'){a=between(2,12);finish(a+'의 なし수를 작은 수부터 3개 書きましょう.','',[a,a*2,a*3].join(', '),a+'에 1, 2, 3を각각 곱합니다.',{wrong:[a,a+1,a+2].join(', ')})}
 else if(kind==='gcd'||kind==='lcm'){a=between(4,24);b=between(4,24);const ans=kind==='gcd'?gcd(a,b):a*b/gcd(a,b);finish(a+'과 '+b+'의 '+(kind==='gcd'?'최대공약수':'최소공なし수')+'는 무엇인가요?','',ans,kind==='gcd'?'공통인 약수 중 가장 큰 수를 찾습니다.':'공통인 なし수 중 가장 작은 수를 찾습니다.')}
 ...
 if(kind==='graph-choice')finish('時間에 따른 기온의 변화를 나타내기에 알맞은 그래프는 무엇인가요?','','꺾은선그래프','時間에 따라 변하는 모습を선으로 연결합니다.',{choices:['꺾은선그래프','원그래프','띠그래프'],wrong:'원그래프'});
 ...
 else if(kind==='triangle-angle'||kind==='quad-angle'){const tri=kind==='triangle-angle',sum=tri?180:360,vals=tri?[between(3,7)*10,between(3,7)*10]:[90,90,between(6,12)*10];finish((tri?'三角形':'四角形')+'의 각 중 '+vals.map(v=>v+'°').join(', ')+'를 알고 있습니다. あまり 각은 몇 도인가요?','',sum-vals.reduce((x,y)=>x+y),'각의 합 '+sum+'°の알려진 각を뺍니다.')}
 else if(kind==='triangle-side'){a=between(3,9);finish('세 변が모두 '+a+' cm인 三角形의 이름은 무엇인가요?',poly(3),'정三角形','세 변의 長さ가 모두 같습니다.',{choices:['정三角形','직각三角形','둔각三角形'],wrong:'둔각三角形'})}
 else if(kind==='triangle-type'){const idx=r(3),angles=[[60,60,60],[30,60,90],[30,30,120]][idx];finish('세 각が'+angles.join('°, ')+'°인 三角形を각에 따라 分류하세요.','',['예각三角形','직각三角形','둔각三角形'][idx],'직각 또는 둔각が있는지 확인합니다.',{choices:['예각三角形','직각三角形','둔각三角形'],wrong:['둔각三角形','예각三角形','직각三角形'][idx]})}
 else if(kind==='quad-type'){const idx=r(4),names=['직四角形','정四角形','마름모','평행사변형'],facts=['네 각が직각이고 가로 6 cm, 세로 3 cm','네 각が직각이고 네 변が모두 4 cm','네 변が같고 한 각が60°','마주 보는 두 쌍의 변が평행하고 이웃한 변が4 cm와 6 cm, 한 각が60°'];finish(facts[idx]+'인 四角形의 이름은 무엇인가요?','',names[idx],'변의 長さ와 각의 조건を확인합니다.',{choices:names,wrong:names[(idx+1)%4]})}
 else if(kind==='parallel')finish('직四角形の마주 보는 두 변의 관계를 書きましょう.',rect('6 cm','3 cm'),'평행','아무리 늘여도 만나지 않는 두 직선です。',{choices:['평행','수직'],wrong:'수직'});
 ...
 if(kind==='circle-parts')finish('중심の원 위의 한 점까지 이은 선分의 이름은 무엇인가요?',visual,'반지름','중심の원 위의 점까지 이은 선分です。',{choices:['반지름','지름','꼭짓점'],wrong:'지름'});
 else if(kind==='circle-size')finish('원의 지름은 몇 cm인가요?',visual,a*2,'지름은 반지름의 2なしです。');
 ...
 else if(kind==='symmetry-line')finish('정四角形의 대칭축은 몇 개인가요?',poly(4),4,'마주 보는 변의 중점を잇는 2개와 대각선 2個です。');
 ...
 if(kind==='perimeter'){result=2*(a+b);stem='직四角形의 둘레는 몇 cm인가요?';art=rect(a+' cm',b+' cm');why='(가로 + 세로) × 2'}
 else if(kind==='surface'){result=2*(a*b+b*c+a*c);stem='가로 '+a+' cm, 세로 '+b+' cm, 높が'+c+' cm인 直方体의 겉広さ는 몇 cm²인가요?';art=cubeArt();why='(가로×세로 + 세로×높が+ 가로×높이) × 2'}
 else if(kind==='volume'){result=a*b*c;stem='가로 '+a+' cm, 세로 '+b+' cm, 높が'+c+' cm인 直方体의 부피는 몇 cm³인가요?';art=cubeArt();why='가로 × 세로 × 높이'}
 else if(kind==='volume-convert'){result=a*1000000;stem=a+' m³는 몇 cm³인가요?';art='';why='1 m³ = 1,000,000 cm³'}
 else {b=2*between(2,5);if(kind==='area-trapezoid')c=a+between(1,5);result=kind==='area-triangle'?a*b/2:kind==='area-trapezoid'?(a+c)*b/2:kind==='area-rhombus'?a*b/2:a*b;stem=kind==='area-triangle'?'밑변 '+a+' cm, 높が'+b+' cm인 三角形':kind==='area-trapezoid'?'윗변 '+a+' cm, 아랫변 '+c+' cm, 높が'+b+' cm인 사다리꼴':kind==='area-rhombus'?'두 대각선が'+a+' cm, '+b+' cm인 마름모':kind==='area-parallelogram'?'밑변 '+a+' cm, 높が'+b+' cm인 평행사변형':'가로 '+a+' cm, 세로 '+b+' cm인 직四角形';stem+='의 広さ는 몇 cm²인가요?';art=kind==='area-rectangle'?rect(a+' cm',b+' cm'):svg('<path d="'+(kind==='area-triangle'?'M35 70L95 12L180 70Z':kind==='area-trapezoid'?'M35 70L65 15H140L180 70Z':kind==='area-rhombus'?'M35 43L105 10L180 43L105 77Z':'M35 70L65 15H185L155 70Z')+'" fill="#e6edf8" stroke="#6d86a5" stroke-width="2"/>'+text(110,91,'長さ는 문제의 조건を보세요'),220,100);why=kind==='area-triangle'?'밑변 × 높が÷ 2':kind==='area-trapezoid'?'(윗변 + 아랫변) × 높が÷ 2':kind==='area-rhombus'?'대각선 × 다른 대각선 ÷ 2':'밑변(가로) × 높이(세로)'}
 ...
 else {const result=kind==='cube-view'?(front?Math.max(hs[0],hs[2])+Math.max(hs[1],hs[3]):Math.max(hs[0],hs[1])+Math.max(hs[2],hs[3])):kind==='cube-missing'?sum-hs[0]:sum;finish(kind==='cube-view'?(front?'앞':'오른쪽 옆')+'の보이는 정四角形은 몇 개인가요?':kind==='cube-missing'?'왼쪽 위 자리의 쌓기나무를 모두 빼면 몇 개가 남나요?':'쌓기나무는 모두 몇 개인가요?',visual,result,kind==='cube-view'?'보는 방향の겹친 줄마다 가장 높은 개수를 더하면 '+result+'個です。':hs.join(' + ')+' = '+sum+(kind==='cube-missing'?', '+sum+' − '+hs[0]+' = '+result:''))}
}
 else if(kind==='cube-net'){const cells=[[1,0],[0,1],[1,1],[2,1],[3,1],[1,2]],idx=r(6);visual=svg(cells.map(([x,y],i)=>'<rect x="'+(30+x*24)+'" y="'+(5+y*24)+'" width="24" height="24" fill="'+(i===idx?'#efca80':'#e7eff5')+'" stroke="#64879a"/>').join(''),170,85);finish('정육면체 전개도の색칠하지 않은 면은 몇 개인가요?',visual,5,'정四角形 6개 중 색칠한 1개를 빼면 5個です。');if(mode===4){prompt='が전개도를 접어 정육면체가 되는 까닭を설명하세요.';answer='6개의 정四角形が겹치지 않고 정육면체의 6개 면を만듭니다.';reason='직접 오려 접어 확인해도 좋습니다.'}}
 else if(['cuboid','cuboid-edges'].includes(kind)){const attr=kind==='cuboid-edges'?['모서리',12]:kind==='cube-net'?['전개도에 필요한 정四角形',6]:pick([['면',6],['꼭짓점',8],['모서리',12]]);finish((kind==='cube-net'?'정육면체':'直方体')+'의 '+attr[0]+'은 몇 개인가요?',cubeArt(),attr[1],'면 6개, 꼭짓점 8개, 모서리 12個です。')}
 ...
 else if(['prism','pyramid'].includes(kind)){const n=between(3,6),pyr=kind==='pyramid',attr=pick(['면','꼭짓점','모서리']),result=attr==='면'?(pyr?n+1:n+2):attr==='꼭짓점'?(pyr?n+1:2*n):(pyr?2*n:3*n),name=['','','','삼','사','오','육'][n]+(pyr?'각뿔':'각기둥');finish(name+'의 '+attr+'은 몇 개인가요?','',result,pyr?'밑면과 꼭대기 꼭짓점を구分하여 셉니다.':'위아래 밑면과 옆면を구分하여 셉니다.')}
 else {const facts={cylinder:['円柱의 밑면은 몇 개인가요?',2,'합동인 원 2個です。'],cone:['원뿔의 꼭짓점은 몇 개인가요?',1,'꼭대기 꼭짓점은 1個です。'],sphere:['구의 중심の겉면까지의 거리는 무엇이라고 하나요?','반지름','반지름은 어느 방향이나 같습니다.'],'cylinder-net':['円柱의 옆면を높が방향으로 잘라 펼치면 무슨 모양인가요?','직四角形','평행한 두 밑면 사이를 높が방향으로 자릅니다.']};const [stem,result,why]=facts[kind];finish(stem,'',result,why,{wrong:typeof result==='number'?result+1:'三角形',choices:typeof result==='number'?undefined:[result,'三角形','꼭짓점']})}
}
