...
[`${a}より ${b} 大きい数はいくつですか。`,c,`${a} + ${b} = ${c}`],
[`${a}が ${c}にするには、あといくつ必要ですか。`,b,`${c} − ${a} = ${b}`],
[`${b}は ${c}よりどれだけ小さいですか。`,a,`${c} − ${b} = ${a}`],
[`二つの量を合わせると ${c}です。一方が ${a}のとき、もう一方はいくつですか。`,b,`${c} − ${a} = ${b}`],
[`${a}にある数をたすと ${c}になりました。ある数はいくつですか。`,b,`${c} − ${a} = ${b}`],
[`${a}あった量に ${b}を加えました。全部でいくつですか。`,c,`${a} + ${b} = ${c}`]];[prompt,answer,work]=items[form];}
