...
const familyNames={'Natural numbers':'Numeri naturali',Fractions:'Frazioni',Decimals:'Numeri decimali','Times tables':'Tabelline'};
const operationNames={Addition:'Addizione',Subtraction:'Sottrazione',Multiplication:'Moltiplicazione',Division:'Divisione','Number sense':'Senso del numero'};
const baseTitles={'add-small':'Addizioni entro 20','add-two':'Addizioni a due cifre senza riporto','add-carry':'Addizioni a due cifre con riporto','subtract-small':'Sottrazioni entro 10','subtract-two':'Sottrazioni a due cifre senza cambio','subtract-borrow':'Sottrazioni a due cifre con cambio','multiply-one':'Moltiplicazioni a una cifra','multiply-two-one':'Due cifre per una cifra','multiply-two':'Due cifre per due cifre',divide:'Divisioni senza resto',missing:'Completa il numero mancante',compare:'Confronta i numeri','compare-ten':'Confronta i numeri entro 10'};
const fractionLabels={whole:'numero intero',proper:'frazione propria',improper:'frazione impropria',mixed:'numero misto'};
for(const t of types){const c=t.config,op=operationNames[t.group]||t.group,dp=n=>n===0?'numero intero':n+' '+(n===1?'cifra decimale':'cifre decimali');
if(c?.kind==='natural')t.title=c.da+' '+(c.da===1?'cifra':'cifre')+' '+c.op+' '+c.db+' '+(c.db===1?'cifra':'cifre');
else if(c?.kind==='decimal')t.title=op+' · '+dp(c.dp);
else if(c?.kind==='decimal-pair')t.title=dp(c.da)+' '+c.op+' '+dp(c.db);
else if(c?.kind==='fraction')t.title=op+' · '+(c.same?'denominatori uguali':'denominatori diversi');
else if(c?.kind==='fraction-pair')t.title=fractionLabels[c.fa]+' '+c.op+' '+fractionLabels[c.fb];
else if(c?.kind==='integer')t.title=op+' con numeri negativi';
else if(baseTitles[t.id])t.title=baseTitles[t.id];
t.instruction=t.group==='Division'?'Esegui le divisioni. Indica il resto, se presente.':t.group==='Number sense'?'Completa con il numero o il segno corretto.':'Calcola e scrivi il risultato.';}
let koBrowse='topic';
function koMenu(){groups.innerHTML='<nav class="edition-tabs"><a href="/it/" aria-current="page"><span class="arithmetic-menu-icons"><span>＋</span><span>−</span><span>×</span><span>÷</span></span><span>Quattro operazioni</span></a><a href="/it/drills.html">Calcolo per argomento</a><a href="/it/units.html">Attività per argomento</a></nav>';
...
function choose(t){if(t.id===current.id)return;location.href=({"add-small":"/it/addizioni-entro-20.html","add-two":"/it/addizioni-senza-riporto.html","add-carry":"/it/addizioni-con-riporto.html","subtract-small":"/it/sottrazioni-entro-10.html","subtract-two":"/it/sottrazioni-senza-cambio.html","subtract-borrow":"/it/sottrazioni-con-cambio.html","multiply-one":"/it/moltiplicazioni-una-cifra.html","multiply-two-one":"/it/moltiplicazioni-due-per-una-cifra.html","multiply-two":"/it/moltiplicazioni-due-cifre.html","divide":"/it/divisioni-senza-resto.html","missing":"/it/numeri-mancanti.html","compare":"/it/confrontare-numeri.html"})[t.id]||('/it/?type='+encodeURIComponent(t.id));}
function menu(){koMenu();}
menu();
const escape=s=>String(s).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;');
let trackedWorksheet=null;
function trackWorksheet(event){window.GDAnalytics?.track(event,{worksheet_id:current.id,worksheet_name:current.title,edition:'it',number_family:current.family,operation:current.group,sheet_mode:answers?'answer':'worksheet'});}
...
function render(){
 if(current.id==='times-tables'){
  current.title='Tabelline · '+GDTimes.selected.join('·')+' : tabelline scelte';GDTimes.decorate(seed);
  document.querySelector('#new').disabled=!GDTimes.selected.length;
  if(!GDTimes.selected.length){document.querySelector('#sheet-title').textContent='Scegli le tabelline';document.querySelector('.problems').innerHTML='';document.querySelector('#print').disabled=true;return;}
 ...
 document.querySelector('#instructions').textContent=answers?(current.family==='Fractions'?'Le frazioni hanno lo stesso valore prima e dopo la semplificazione.':'Soluzioni e passaggi per gli stessi esercizi.'):current.instruction;
 document.querySelector('#mode').textContent=answers?'Soluzioni':'Scheda';
 ...
 document.querySelector('#set').textContent=`Serie ${seed} · ${answers?'Soluzioni':count+' esercizi'}`;
 document.querySelector('#status').textContent=current.title+(answers?' · Soluzioni':' · '+count+' esercizi pronti');
}
