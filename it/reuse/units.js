...
function title(){if(window.WORKSHEET_ENTRY?.assessment)return "Verifica";return M.profiles(unit.id)[profile].name}
...
function fit(){
 ...
 holder.remove();
 if(!best)throw Error('La scheda supera una pagina: '+unit.id+' / '+profile);
...
function draw(){
 ...
 $('#status').textContent=counts.reduce((a,b)=>a+b,0)+' esercizi · '+sections.length+' attività · scheda e soluzioni';
}
function render(){
 profile=Math.min(profile,M.profiles(unit.id).length-1);sections=window.WORKSHEET_ENTRY?.assessment?KoAssessmentBase.generate(unit.id,0,seed,4,unit.skills.map((skill,i)=>({skill,mode:i%2}))):M.generate(unit.id,profile,seed,8);if(window.WORKSHEET_ENTRY?.assessment){const all=sections;sections=[];for(let i=0;i<all.length;i+=2){sections.push({title:'Ripasso '+(i/2+1),questions:Array.from({length:4},(_,j)=>all.slice(i,i+2).map(x=>x.questions[j])).flat()});}}fit();draw();
...
function goProfile(p){if(browseUnit.id===unit.id&&p===profile)return;location.assign('/it/print/unit-'+browseUnit.id+'-'+p+'.html');}
...
$('#new').onclick=()=>{window.GDAnalytics?.track('worksheet_regenerate',{worksheet_id:unit.id+'-'+profile,edition:'it'});M.excludeArt([...document.querySelectorAll('#sheet-view [data-art-id]')].map(x=>x.dataset.artId));seed=(seed+1+Math.floor(Math.random()*99999999))%100000000;render()};
$('#worksheet').onclick=()=>{answer=false;draw()};$('#answer').onclick=()=>{answer=true;draw()};
function prepare(){if(!$('#print-q').checked&&!$('#print-a').checked){$('#print-bundle').innerHTML='';$('#status').textContent='Scegli la scheda o le soluzioni da stampare.';return false}const originalSeed=seed,copies=Math.min(20,Math.max(1,Math.floor(Number(document.querySelector('#worksheet-copies')?.value)||1)));let pages='';try{for(let copy=0;copy<copies;copy++){seed=(originalSeed+copy)>>>0;render();pages+=($('#print-q').checked?sheetHTML(false):'')+($('#print-a').checked?sheetHTML(true):'');}}finally{seed=originalSeed;render();}$('#print-bundle').innerHTML=pages;return true}
