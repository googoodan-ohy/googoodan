...
addEventListener('DOMContentLoaded',()=>{
 ...
 const pdf=document.createElement('button');pdf.type='button';pdf.id='save-pdf';pdf.textContent=(en?"Save PDF":"PDF 저장");pdf.title=(en?"Create and save a PDF file":"PDF 파일을 만들어 저장합니다");
 ...
 print.after(pdf);
 const quantity=document.createElement('label');quantity.style.cssText='display:inline-flex;align-items:center;gap:6px;font-size:12px;white-space:nowrap';quantity.append((en?"Copies":"매수"));
 const count=document.createElement('input');count.id='worksheet-copies';count.type='number';count.min='1';count.max='20';count.step='1';count.value='1';count.setAttribute('aria-label',(en?"Number of different worksheets":"서로 다른 문제지 매수"));count.title=(en?"1–20 worksheets. Include answers to add the same number of answer sheets.":"1~20매. 정답지 선택 시 같은 수의 정답지가 추가됩니다.");count.style.cssText='width:50px;padding:8px 4px;border:1px solid #bfd8d7;border-radius:5px;font:inherit';
 const arrows=document.createElement('span');arrows.style.cssText='display:flex;flex-direction:column';for(const [label,delta]of [['▲',1],['▼',-1]]){const b=document.createElement('button');b.type='button';b.textContent=label;b.setAttribute('aria-label',delta>0?(en?"Increase copies":"매수 늘리기"):(en?"Decrease copies":"매수 줄이기"));b.style.cssText='padding:0 6px;font-size:10px;line-height:16px;border:1px solid #bfd8d7;background:white;color:#176e5c';b.onclick=()=>{count.value=String(Math.min(20,Math.max(1,(Number(count.value)||1)+delta)));};arrows.append(b);}
 ...
 const hint=document.createElement('p');hint.hidden=true;hint.setAttribute('role','status');hint.style.cssText='font-size:12px;color:#42665d;margin:8px 0';hint.textContent=(en?"Choose Save as PDF in the print dialog to save the selected worksheets and answers.":"인쇄창의 대상(프린터)에서 “PDF로 저장”을 선택하세요. 선택한 문제지·정답지가 파일로 저장됩니다.");
 (print.closest('.toolbar')||print.parentElement).after(hint);
 const description=document.createElement('p');description.className='pdf-save-note';description.style.cssText='font-size:12px;line-height:1.7;color:#42665d;margin:8px 0';description.textContent=(en?"Save different problems for each copy in one PDF. For example, 3 copies creates 3 different worksheets. Include answers to add 3 answer sheets.":"선택한 매수만큼 숫자가 서로 다른 문제지를 만들어 하나의 PDF 파일로 저장합니다. 예: 3매 선택 → 다른 문제지 3장. 정답지 선택 시 정답 3장이 추가됩니다.");hint.after(description);
