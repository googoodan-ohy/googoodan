...
const canonical=document.querySelector('link[rel=canonical]');if(canonical)canonical.href=window.WORKSHEET_ENTRY?.assessment?'https://googoodan.com'+location.pathname:'https://googoodan.com'+'/ko/print/unit-'+unit.id+'-'+profile+'.html';
 renderProfileMenu();
...
// Preview follows the selected output; prepare() can still render both modes.
const printQuestions=$('#print-q'),printAnswers=$('#print-a');
function syncOutputPreview(){
 if(!window.GDPDFBusy)$('#print-bundle').replaceChildren();
 if(printQuestions.checked||printAnswers.checked)answer=printAnswers.checked;
 $('#print').disabled=!printQuestions.checked&&!printAnswers.checked;
 const pdf=$('#save-pdf');if(pdf)pdf.disabled=$('#print').disabled||!!window.GDPDFBusy;
 draw();
}
$('#worksheet').onclick=()=>{printQuestions.checked=true;printAnswers.checked=false;syncOutputPreview()};
$('#answer').onclick=()=>{printQuestions.checked=false;printAnswers.checked=true;syncOutputPreview()};
function prepare(){if(!$('#print-q').checked&&!$('#print-a').checked){$('#print-bundle').innerHTML='';$('#status').textContent='인쇄할 문제지 또는 정답지를 선택해 주세요.';return false}const originalSeed=seed,copies=window.GDPDFBusy?Math.min(20,Math.max(1,Math.floor(Number(document.querySelector('#worksheet-copies')?.value)||1))):1;let pages='';try{for(let copy=0;copy<copies;copy++){seed=(originalSeed+copy)>>>0;render();pages+=($('#print-q').checked?sheetHTML(false):'')+($('#print-a').checked?sheetHTML(true):'');}}finally{seed=originalSeed;render();}$('#print-bundle').innerHTML=pages;return true}
...
menus();document.fonts.ready.then(()=>{render();syncOutputPreview()});
})();
