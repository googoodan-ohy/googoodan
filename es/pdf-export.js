(()=>{
 ...
 const loads=new Map();function load(src){if(!loads.has(src))loads.set(src,new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.onload=resolve;s.onerror=()=>reject(Error((en?"Could not load the PDF tools.":"No se pudieron cargar las herramientas PDF.")));document.head.append(s);}));return loads.get(src);}
 window.saveWorksheetPDF=async function(button){
  ...
  const track=name=>{let c={};try{c=window.GDWorksheetContext?.()||{};}catch{}window.GDAnalytics?.track(name,{edition:en?'en':'es',worksheet_id:c.drill||c.unit||c.type||location.pathname});};track('worksheet_pdf_click');
  ...
  try{
   ...
   for(let i=0;i<pages.length;i++){
    button.textContent=(en?"Creating PDF ":"Creando PDF ")+(i+1)+'/'+pages.length;
    ...
    if(!canvas.width||!canvas.height)throw Error((en?"Could not determine the PDF page size: ":"No se pudo determinar el tamaño de la página PDF: ")+pages[i].getBoundingClientRect().width+' × '+pages[i].getBoundingClientRect().height);
   ...
   const msg=document.querySelector('.pdf-save-note');if(msg)msg.textContent=pages.length+(en?" pages prepared as a PDF. Check your downloads or selected location. Each copy contains different problems.":" páginas PDF creadas. Cada ficha contiene números diferentes. Revisa la carpeta de descargas.");
  }catch(e){if(e.name!=='AbortError'){track('worksheet_pdf_error');const msg=document.querySelector('.pdf-save-note');if(msg)msg.textContent=(en?"PDF save failed: ":"No se pudo guardar el PDF: ")+e.message;}}
