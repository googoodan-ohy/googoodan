...
function title(){if(window.WORKSHEET_ENTRY?.assessment)return "Evaluación";return M.profiles(unit.id)[profile].name}
...
function fit(){
 ...
 holder.remove();
 if(!best)throw Error('La ficha supera una página: '+unit.id+' / '+profile);
...
function draw(){
 ...
 $('#status').textContent=counts.reduce((a,b)=>a+b,0)+' ejercicios · '+sections.length+' actividades · ficha y soluciones';
}
function render(){
 profile=Math.min(profile,M.profiles(unit.id).length-1);sections=window.WORKSHEET_ENTRY?.assessment?KoAssessmentBase.generate(unit.id,0,seed,4,unit.skills.map((skill,i)=>({skill,mode:i%2}))):M.generate(unit.id,profile,seed,8);if(window.WORKSHEET_ENTRY?.assessment){const all=sections;sections=[];for(let i=0;i<all.length;i+=2){sections.push({title:'Repaso '+(i/2+1),questions:Array.from({length:4},(_,j)=>all.slice(i,i+2).map(x=>x.questions[j])).flat()});}}fit();draw();
...
function goProfile(p){if(browseUnit.id===unit.id&&p===profile)return;location.assign('/es/print/unit-'+browseUnit.id+'-'+p+'.html');}
...
$('#new').onclick=()=>{window.GDAnalytics?.track('worksheet_regenerate',{worksheet_id:unit.id+'-'+profile,edition:'es'});M.excludeArt([...document.querySelectorAll('#sheet-view [data-art-id]')].map(x=>x.dataset.artId));seed=(seed+1+Math.floor(Math.random()*99999999))%100000000;render()};
$('#worksheet').onclick=()=>{answer=false;draw()};$('#answer').onclick=()=>{answer=true;draw()};
function prepare(){if(!$('#print-q').checked&&!$('#print-a').checked){$('#print-bundle').innerHTML='';$('#status').textContent='Elige la ficha o las soluciones para imprimir.';return false}const originalSeed=seed,copies=Math.min(20,Math.max(1,Math.floor(Number(document.querySelector('#worksheet-copies')?.value)||1)));let pages='';try{for(let copy=0;copy<copies;copy++){seed=(originalSeed+copy)>>>0;render();pages+=($('#print-q').checked?sheetHTML(false):'')+($('#print-a').checked?sheetHTML(true):'');}}finally{seed=originalSeed;render();}$('#print-bundle').innerHTML=pages;return true}
