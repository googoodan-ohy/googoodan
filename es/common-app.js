...
const familyNames={'Natural numbers':'Números naturales',Fractions:'Fracciones',Decimals:'Números decimales','Times tables':'Tablas de multiplicar'};
const operationNames={Addition:'Suma',Subtraction:'Resta',Multiplication:'Multiplicación',Division:'División','Number sense':'Sentido numérico'};
const baseTitles={'add-small':'Sumas hasta 20','add-two':'Sumas de dos cifras sin llevadas','add-carry':'Sumas de dos cifras con llevadas','subtract-small':'Restas hasta 10','subtract-two':'Restas de dos cifras sin llevadas','subtract-borrow':'Restas de dos cifras con llevadas','multiply-one':'Multiplicaciones de una cifra','multiply-two-one':'Dos cifras por una cifra','multiply-two':'Dos cifras por dos cifras',divide:'Divisións exactas',missing:'Completa el número que falta',compare:'Compara los números','compare-ten':'Compara números hasta 10'};
const fractionLabels={whole:'número entero',proper:'fracción propia',improper:'fracción impropia',mixed:'número mixto'};
for(const t of types){const c=t.config,op=operationNames[t.group]||t.group,dp=n=>n===0?'número entero':n+' '+(n===1?'cifra decimal':'cifras decimales');
if(c?.kind==='natural')t.title=c.da+' '+(c.da===1?'cifra':'cifras')+' '+c.op+' '+c.db+' '+(c.db===1?'cifra':'cifras');
else if(c?.kind==='decimal')t.title=op+' · '+dp(c.dp);
else if(c?.kind==='decimal-pair')t.title=dp(c.da)+' '+c.op+' '+dp(c.db);
else if(c?.kind==='fraction')t.title=op+' · '+(c.same?'denominadores iguales':'denominadores distintos');
else if(c?.kind==='fraction-pair')t.title=fractionLabels[c.fa]+' '+c.op+' '+fractionLabels[c.fb];
else if(c?.kind==='integer')t.title=op+' con números negativos';
else if(baseTitles[t.id])t.title=baseTitles[t.id];
t.instruction=t.group==='Division'?'Resuelve las divisiones. Escribe el resto si lo hay.':t.group==='Number sense'?'Completa con el número o el signo correcto.':'Calcula y escribe el resultado.';}
let koBrowse='topic';
function koMenu(){groups.innerHTML='<nav class="edition-tabs"><a href="/es/" aria-current="page"><span class="arithmetic-menu-icons"><span>＋</span><span>−</span><span>×</span><span>÷</span></span><span>Cuatro operaciones</span></a><a href="/es/drills.html">Cálculo por temas</a><a href="/es/units.html">Actividades por temas</a></nav>';
...
function mathHTML(x){
 const text=String(x).replace(' R ',' resto ');if(!text.includes('/'))return text.startsWith('-')?'('+text+')':text;
...
function exampleHTML(t){
 ...
 if(t.family==='Natural numbers'&&op==='÷'&&n%d)result=Math.floor(n/d)+' resto '+n%d;
 const html=mathHTML(left)+' '+op+' '+mathHTML(right)+' = '+green(t.family==='Fractions'?fractionAnswer({a:left,b:right,op}):mathHTML(result));
 return t.family==='Decimals'?html.replace(/(\d)\.(\d)/g,'$1,$2'):html;
}
...
function choose(t){if(t.id===current.id)return;location.href=({"add-small":"/es/sumas-hasta-20.html","add-two":"/es/sumas-sin-llevadas.html","add-carry":"/es/sumas-con-llevadas.html","subtract-small":"/es/restas-hasta-10.html","subtract-two":"/es/restas-sin-llevadas.html","subtract-borrow":"/es/restas-con-llevadas.html","multiply-one":"/es/multiplicaciones-una-cifra.html","multiply-two-one":"/es/multiplicaciones-dos-por-una-cifra.html","multiply-two":"/es/multiplicaciones-dos-cifras.html","divide":"/es/divisiones-exactas.html","missing":"/es/numeros-que-faltan.html","compare":"/es/comparar-numeros.html"})[t.id]||('/es/?type='+encodeURIComponent(t.id));}
function menu(){koMenu();}
menu();
const escape=s=>String(s).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;');
let trackedWorksheet=null;
function trackWorksheet(event){window.GDAnalytics?.track(event,{worksheet_id:current.id,worksheet_name:current.title,edition:'es',number_family:current.family,operation:current.group,sheet_mode:answers?'answer':'worksheet'});}
function writtenWork(p,show){
 ...
 return note+'<svg class="long-division" viewBox="0 0 '+width+' '+height+'" style="height:'+height+'px" aria-label="División por escrito: '+p.a+' entre '+p.b+'"><g text-anchor="end" font-family="monospace" font-size="19" fill="#243e55">'+txt(decimal&&!show?p.b:b,x0-9,startY)+dividend+'<path d="M'+(x0-2)+' '+(startY+5)+'V29H'+(width-3)+'" fill="none" stroke="#243e55"/><g class="working '+(show?'':'concealed')+'" fill="#c62828" stroke-width="1">'+top+'<g stroke="#c62828">'+body.replaceAll('<text ','<text stroke="none" ')+'</g></g></g></svg>';
}

function render(){
 if(current.id==='times-tables'){
  current.title='Tablas · '+GDTimes.selected.join('·')+' : tablas elegidas';GDTimes.decorate(seed);
  document.querySelector('#new').disabled=!GDTimes.selected.length;
  if(!GDTimes.selected.length){document.querySelector('#sheet-title').textContent='Elige las tablas';document.querySelector('.problems').innerHTML='';document.querySelector('#print').disabled=true;return;}
 ...
 document.querySelector('#instructions').textContent=answers?(current.family==='Fractions'?'Las fracciones tienen el mismo valor antes y después de simplificar.':'Soluciones y pasos de los mismos ejercicios.'):current.instruction;
 document.querySelector('#mode').textContent=answers?'Soluciones':'Ficha';
 ...
 if(current.family==='Decimals')html=html.replace(/(\d)\.(\d)/g,'$1,$2');return `<div class="problem ${p.op==='missing'||p.op==='compare'?'concept':''}"><span class="number">${i+1}.</span>${html}</div>`;
 }).join('');
 document.querySelector('#set').textContent=`Serie ${seed} · ${answers?'Soluciones':count+' ejercicios'}`;
 document.querySelector('#status').textContent=current.title+(answers?' · Soluciones':' · '+count+' ejercicios preparados');
}
