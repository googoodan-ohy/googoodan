...
const familyNames={'Natural numbers':'Natürliche Zahlen',Fractions:'Brüche',Decimals:'Dezimalzahlen','Times tables':'Einmaleins'};
const operationNames={Addition:'Addition',Subtraction:'Subtraktion',Multiplication:'Multiplikation',Division:'Division','Number sense':'Zahlenverständnis'};
const baseTitles={'add-small':'Addition bis 20','add-two':'Zweistellige Addition ohne Übergang','add-carry':'Zweistellige Addition mit Übergang','subtract-small':'Subtraktion bis 10','subtract-two':'Zweistellige Subtraktion ohne Übergang','subtract-borrow':'Zweistellige Subtraktion mit Übergang','multiply-one':'Einstellige Multiplikation','multiply-two-one':'Zweistellig mal einstellig','multiply-two':'Zweistellig mal zweistellig',divide:'Division ohne Rest',missing:'Fehlende Zahl ergänzen',compare:'Zahlen vergleichen','compare-ten':'Zahlen bis 10 vergleichen'};
const fractionLabels={whole:'ganze Zahl',proper:'echter Bruch',improper:'unechter Bruch',mixed:'gemischte Zahl'};
for(const t of types){const c=t.config,op=operationNames[t.group]||t.group,dp=n=>n===0?'ganze Zahl':n+' Nachkommastelle'+(n===1?'':'n');
if(c?.kind==='natural')t.title=c.da+'-stellig '+c.op+' '+c.db+'-stellig';
else if(c?.kind==='decimal')t.title=op+' · '+dp(c.dp);
else if(c?.kind==='decimal-pair')t.title=dp(c.da)+' '+c.op+' '+dp(c.db);
else if(c?.kind==='fraction')t.title=op+' · '+(c.same?'gleiche Nenner':'verschiedene Nenner');
else if(c?.kind==='fraction-pair')t.title=fractionLabels[c.fa]+' '+c.op+' '+fractionLabels[c.fb];
else if(c?.kind==='integer')t.title=op+' mit negativen Zahlen';
...
function mathHTML(x){
 const text=String(x).replace(' R ',' Rest ');if(!text.includes('/'))return text.startsWith('-')?'('+text+')':text;
...
function exampleHTML(t){
 ...
 if(t.family==='Natural numbers'&&op==='÷'&&n%d)result=Math.floor(n/d)+' Rest '+n%d;
...
function choose(t){if(t.id===current.id)return;location.href=({"add-small":"/de/addition-bis-20.html","add-two":"/de/addition-ohne-uebertrag.html","add-carry":"/de/addition-mit-uebertrag.html","subtract-small":"/de/subtraktion-bis-10.html","subtract-two":"/de/subtraktion-ohne-uebertrag.html","subtract-borrow":"/de/subtraktion-mit-uebertrag.html","multiply-one":"/de/multiplikation-einstellig.html","multiply-two-one":"/de/multiplikation-zweistellig-einstellig.html","multiply-two":"/de/multiplikation-zweistellig.html","divide":"/de/division-ohne-rest.html","missing":"/de/fehlende-zahlen.html","compare":"/de/zahlen-vergleichen.html"})[t.id]||('/de/?type='+encodeURIComponent(t.id));}
function menu(){koMenu();}
menu();
const escape=s=>String(s).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;');
let trackedWorksheet=null;
function trackWorksheet(event){window.GDAnalytics?.track(event,{worksheet_id:current.id,worksheet_name:current.title,edition:'de',number_family:current.family,operation:current.group,sheet_mode:answers?'answer':'worksheet'});}
...
function render(){
 if(current.id==='times-tables'){
  current.title='Einmaleins · '+GDTimes.selected.join('·')+' : ausgewählte Reihen';GDTimes.decorate(seed);
  document.querySelector('#new').disabled=!GDTimes.selected.length;
  if(!GDTimes.selected.length){document.querySelector('#sheet-title').textContent='Wähle die Einmaleinsreihen';document.querySelector('.problems').innerHTML='';document.querySelector('#print').disabled=true;return;}
 ...
 document.querySelector('#instructions').textContent=answers?(current.family==='Fractions'?'Die Brüche haben vor und nach dem Kürzen denselben Wert.':'Lösungen und Rechenwege zu denselben Aufgaben.'):current.instruction;
 document.querySelector('#mode').textContent=answers?'Lösungen':'Arbeitsblatt';
 ...
 document.querySelector('#set').textContent=`Set ${seed} · ${answers?'Lösungen':count+' Aufgaben'}`;
 document.querySelector('#status').textContent=current.title+(answers?' · Lösungen':' · '+count+' Aufgaben bereit');
}
