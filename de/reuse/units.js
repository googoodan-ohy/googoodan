(function(){
const C=KoCatalog,M=KoMath,$=s=>document.querySelector(s),E=s=>String(s).replace(/(-?\d+) +(\d+\/\d+)/g,'$1 $2').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;');
...
function title(){if(window.WORKSHEET_ENTRY?.assessment)return "Évaluation";return M.profiles(unit.id)[profile].name}
function mixedMarkup(html){return String(html).split(/(<[^>]*>)/g).map((s,i)=>i%2?s:s.replace(/(-?\d+) +(\d+\/\d+)/g,'$1 $2')).join('')}
function qHTML(q,i){return '<div class="question" data-method="'+E(q.methodId||q.skill)+'" data-art="'+E(q.artId||'')+'"><span class="qnumber">'+i+'</span><div class="prompt">'+E(q.prompt)+'</div><div class="visual">'+GDAnswerBoxes(mixedMarkup(q.visual))+'</div><div class="task">'+GDAnswerBoxes(mixedMarkup(q.task))+'</div><div class="solution">'+E(q.answer)+'<span class="explanation">'+E(q.reason)+'</span></div></div>'}
function sheetHTML(isAnswer){
 let n=0;const t=theme();
 return '<article class="sheet '+(compact?'compact ':'')+(isAnswer?'answer-key':'')+'" data-sections="'+sections.length+'" data-grade="'+grade+'" style="--accent:'+t[2]+';--light:'+t[3]+'"><div class="sheet-top"><span class="worksheet-brand"><small>Kostenlose Aufgaben zum Üben · </small><b>googoodan</b> <small>· googoodan.com</small></span><span>'+(unit.gradeLabel+' · ')+(isAnswer?'Lösungen und Erklärungen':'Mein Mathe-Arbeitsblatt')+'</span></div><div class="hero"><div><small>'+E(t[0])+'</small><h2>'+unit.number+'. '+E(unit.name)+'</h2><small>'+title()+' · '+E(t[1])+'</small></div>'+mascot(t[4])+'</div><div class="fields"><span>Name: __________________</span><span>Datum: ______________</span></div><div class="activities">'+sections.map((s,i)=>'<section class="activity"><h3><span>'+(i+1)+'</span>'+E(s.title)+'</h3><div class="question-grid">'+s.questions.slice(0,counts[i]).map(q=>qHTML(q,++n)).join('')+'</div></section>').join('')+'</div><footer class="sheet-footer"><b>Kostenlose Aufgaben zum Üben · googoodan · googoodan.com</b><span>'+unit.id+' / '+(profile+1)+' · '+seed+'</span></footer><div class="art-credit">Bilder: Twemoji © Twitter und Mitwirkende · CC BY 4.0 · googoodan.com/ko/art-gallery.html</div></article>';
}
function fit(){
 ...
 holder.remove();
 if(!best)throw Error('Das Arbeitsblatt überschreitet eine Seite: '+unit.id+' / '+profile);
...
function draw(){
 ...
 $('#status').textContent=counts.reduce((a,b)=>a+b,0)+' Aufgaben · '+sections.length+' Aktivitäten · Arbeitsblatt und Lösungen';
}
function render(){
 profile=Math.min(profile,M.profiles(unit.id).length-1);sections=window.WORKSHEET_ENTRY?.assessment?KoAssessmentBase.generate(unit.id,0,seed,4,unit.skills.map((skill,i)=>({skill,mode:i%2}))):M.generate(unit.id,profile,seed,8);if(window.WORKSHEET_ENTRY?.assessment){const all=sections;sections=[];for(let i=0;i<all.length;i+=2){sections.push({title:'Révision '+(i/2+1),questions:Array.from({length:4},(_,j)=>all.slice(i,i+2).map(x=>x.questions[j])).flat()});}}fit();draw();
 const t=theme();document.body.style.setProperty('--accent',t[2]);document.body.style.setProperty('--light',t[3]);
 $('#theme-scene').innerHTML=KoThemeScene(t[4],mascot);$('#theme-scene').setAttribute('aria-label',t[0]+' illustration');$('#theme-name').textContent=t[0];$('#theme-motto').textContent=t[1];
 $('#coverage').textContent=sections.map(s=>s.title).join(' · ');
 // Preserve the static title.
...
function goProfile(p){if(browseUnit.id===unit.id&&p===profile)return;location.assign('/de/print/unit-'+browseUnit.id+'-'+p+'.html');}
...
function menus(){
 $('#grades').innerHTML=[...new Set(C.units.map(u=>u.grade))].map(g=>'<button data-g="'+g+'" aria-pressed="'+(g===browseGrade)+'">'+(C.units.find(u=>u.grade===g).gradeLabel)+'</button>').join('');
 ...
 $('#terms').innerHTML=[].map(t=>'<button data-t="'+t+'" aria-pressed="'+(t===browseSemester)+'">'+t+'</button>').join('');
...
$('#new').onclick=()=>{window.GDAnalytics?.track('worksheet_regenerate',{worksheet_id:unit.id+'-'+profile,edition:'de'});M.excludeArt([...document.querySelectorAll('#sheet-view [data-art-id]')].map(x=>x.dataset.artId));seed=(seed+1+Math.floor(Math.random()*99999999))%100000000;render()};
$('#worksheet').onclick=()=>{answer=false;draw()};$('#answer').onclick=()=>{answer=true;draw()};
function prepare(){if(!$('#print-q').checked&&!$('#print-a').checked){$('#print-bundle').innerHTML='';$('#status').textContent='Wähle Arbeitsblatt oder Lösungen zum Drucken.';return false}const originalSeed=seed,copies=Math.min(20,Math.max(1,Math.floor(Number(document.querySelector('#worksheet-copies')?.value)||1)));let pages='';try{for(let copy=0;copy<copies;copy++){seed=(originalSeed+copy)>>>0;render();pages+=($('#print-q').checked?sheetHTML(false):'')+($('#print-a').checked?sheetHTML(true):'');}}finally{seed=originalSeed;render();}$('#print-bundle').innerHTML=pages;return true}
...
const host=document.createElement('div');document.querySelector('.unit-edition-tabs').after(host);GDRegions.mount(host);const rs=host.querySelector('select');if(!rs.value){rs.value='BY';sessionStorage.setItem('ggd-de-region','BY');}const regionNote=document.createElement('p');host.after(regionNote);function updateRegion(){const ok=rs.value==='BY';for(const sel of ['#grades','#terms','#unit','#profile-dots','label[for=unit]'])$(sel).hidden=!ok;regionNote.textContent=ok?'Bayern · Ausgewählte Übungen für Klasse 1/2 und 3/4':'Für dieses Bundesland sind noch keine Kapitel zugeordnet. Die Vorschau gehört zu Bayern.';}rs.addEventListener('change',updateRegion);menus();updateRegion();document.fonts.ready.then(render);
})();
