(()=>{
 ...
 const loads=new Map();function load(src){if(!loads.has(src))loads.set(src,new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.onload=resolve;s.onerror=()=>reject(Error((en?"Could not load the PDF tools.":"PDF-Werkzeuge konnten nicht geladen werden.")));document.head.append(s);}));return loads.get(src);}
 window.saveWorksheetPDF=async function(button){
  ...
  const track=name=>{let c={};try{c=window.GDWorksheetContext?.()||{};}catch{}window.GDAnalytics?.track(name,{edition:en?'en':'de',worksheet_id:c.drill||c.unit||c.type||location.pathname});};track('worksheet_pdf_click');
  const filename=(document.querySelector('#sheet-title,#profile-name')?.textContent||document.title).replace(/[\\/:*?"<>|]/g,' ').slice(0,90)+'.pdf';
  try{
   if('showSaveFilePicker' in window){try{handle=await window.showSaveFilePicker({suggestedName:filename,types:[{description:(en?"PDF document":"PDF-Dokument"),accept:{'application/pdf':['.pdf']}}]});}catch(e){if(e.name==='AbortError'){track('worksheet_pdf_cancel');return;}if(e.name!=='SecurityError'&&e.name!=='NotAllowedError')throw e;}}
   button.disabled=true;button.textContent=(en?"Preparing PDF…":"PDF wird vorbereitet…");
   if(typeof window.GDPreparePDF!=='function')throw Error((en?"PDF saving is not available on this page yet.":"PDF-Speichern ist hier nicht verfügbar."));
   window.GDPreparePDF();const bundle=document.querySelector('#print-bundle');if(!bundle?.children.length)throw Error((en?"Select worksheets or answer sheets to save.":"Bitte Aufgaben oder Lösungen auswählen."));
   await Promise.all([load('/vendor/html2canvas.min.js'),load('/vendor/jspdf.umd.min.js')]);
   const css=await Promise.all([...document.querySelectorAll('link[rel=stylesheet]')].map(async link=>{const r=await fetch(link.href);if(!r.ok)throw Error((en?"Could not load the worksheet styles.":"Arbeitsblatt-Stile konnten nicht geladen werden."));return (await r.text()).replace(/url\((?!["']?(?:data:|https?:|\/|#))(["']?)([^)'" ]+)\1\)/g,(_,q,url)=>'url("'+new URL(url,link.href).href+'")');}));
   ...
   for(let i=0;i<pages.length;i++){
    button.textContent=(en?"Creating PDF ":"PDF wird erstellt ")+(i+1)+'/'+pages.length;
    ...
    if(!canvas.width||!canvas.height)throw Error((en?"Could not determine the PDF page size: ":"PDF-Seitengröße konnte nicht ermittelt werden: ")+pages[i].getBoundingClientRect().width+' × '+pages[i].getBoundingClientRect().height);
   ...
   const msg=document.querySelector('.pdf-save-note');if(msg)msg.textContent=pages.length+(en?" pages prepared as a PDF. Check your downloads or selected location. Each copy contains different problems.":" PDF-Seiten erstellt. Jedes Set enthält andere Zahlen. Bitte den Download-Ordner prüfen.");
  }catch(e){if(e.name!=='AbortError'){track('worksheet_pdf_error');const msg=document.querySelector('.pdf-save-note');if(msg)msg.textContent=(en?"PDF save failed: ":"PDF-Speichern fehlgeschlagen: ")+e.message;}}
