(()=>{
 const loads=new Map();function load(src){if(!loads.has(src))loads.set(src,new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.onload=resolve;s.onerror=()=>reject(Error('PDF 도구를 불러오지 못했습니다.'));document.head.append(s);}));return loads.get(src);}
 window.saveWorksheetPDF=async function(button){
  ...
  try{
   if('showSaveFilePicker' in window){try{handle=await window.showSaveFilePicker({suggestedName:filename,types:[{description:'PDF 문서',accept:{'application/pdf':['.pdf']}}]});}catch(e){if(e.name==='AbortError')return;if(e.name!=='SecurityError'&&e.name!=='NotAllowedError')throw e;}}
   button.disabled=true;button.textContent='PDF 준비 중…';
   if(typeof window.GDPreparePDF!=='function')throw Error('이 화면은 아직 PDF 저장 준비 중입니다.');
   window.GDPreparePDF();const bundle=document.querySelector('#print-bundle');if(!bundle?.children.length)throw Error('저장할 문제지 또는 정답지를 선택해 주세요.');
   await Promise.all([load('/vendor/html2canvas.min.js'),load('/vendor/jspdf.umd.min.js')]);
   const css=await Promise.all([...document.querySelectorAll('link[rel=stylesheet]')].map(async link=>{const r=await fetch(link.href);if(!r.ok)throw Error('문제지 스타일을 불러오지 못했습니다.');return (await r.text()).replace(/url\((?!["']?(?:data:|https?:|\/|#))(["']?)([^)'" ]+)\1\)/g,(_,q,url)=>'url("'+new URL(url,link.href).href+'")');}));
   ...
   for(let i=0;i<pages.length;i++){
    button.textContent='PDF 생성 '+(i+1)+'/'+pages.length;
    ...
    if(!canvas.width||!canvas.height)throw Error('PDF 페이지 크기를 계산하지 못했습니다: '+pages[i].getBoundingClientRect().width+' × '+pages[i].getBoundingClientRect().height);
   ...
   const msg=document.querySelector('.pdf-save-note');if(msg)msg.textContent=pages.length+'페이지 PDF 파일을 저장했습니다. 선택한 매수마다 숫자가 다른 문제가 들어 있습니다.';
  }catch(e){if(e.name!=='AbortError'){const msg=document.querySelector('.pdf-save-note');if(msg)msg.textContent='PDF 저장 실패: '+e.message;}}
