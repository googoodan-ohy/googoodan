(()=>{
const box='<span class="write-answer-box" role="img" aria-label="Blank answer space"></span>';
...
function apply(root){const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);const nodes=[];while(walker.nextNode()){const n=walker.currentNode,p=n.parentElement;if(n.data.includes('□')&&p&&p.closest('.paper,.sheet')&&!p.closest('svg,script,style,.prompt,.solution,.answer,.bank-answer,.write-answer-box,h1,h2,h3,.bank-question>p')&&p.closest('.task,.visual,.expression,.equation,.problem,.question,.kequation,.bank-question'))nodes.push(n);}for(const n of nodes){const t=document.createElement('template');t.innerHTML=GDAnswerBoxes(n.data.replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;'));n.replaceWith(t.content);}}
